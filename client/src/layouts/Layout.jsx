...
export default function Layout({ children }) {
  const [searchOpen,setSearchOpen]=useState(false); const [sidebarOpen,setSidebarOpen]=useState(false);
  ...
  return <div className="min-h-screen text-white" style={{background:'#0B0F14'}}>
    ...
    {sidebarOpen&&<div className="fixed inset-0 z-[70] hidden lg:block"><div className="absolute inset-0 bg-black/60" onClick={()=>setSidebarOpen(false)}/><aside className="absolute inset-y-0 left-0 w-[280px] border-r border-white/[0.07] bg-[#0B0F14] px-4 py-6 shadow-2xl"><div className="flex items-center justify-between px-3 mb-8"><Link to="/" onClick={()=>setSidebarOpen(false)}><img src="/emet-wordmark-white.svg" alt="Emet" className="h-10 w-auto"/></Link><Button variant="custom" size="none" onClick={()=>setSidebarOpen(false)} className="p-2 text-white/60" aria-label="Close navigation"><FiX className="h-5 w-5"/></Button></div><nav className="flex flex-col gap-1">{navItems.map(item=>{const Icon=item.icon;return <NavLink key={item.path} to={item.path} end={item.path==='/'} onClick={()=>setSidebarOpen(false)} className={({isActive})=>`flex items-center gap-4 rounded-2xl px-4 py-3.5 text-[0.95rem] font-semibold ${isActive?'bg-white/[.09] text-white':'text-white/55 hover:bg-white/[.05] hover:text-white'}`}><Icon className="h-[21px] w-[21px]"/>{item.name}</NavLink>;})}{user&&desktopLink('/notifications',FiBell,'Notifications')}{user&&desktopLink('/messages',FiMessageCircle,'Messages')}{user&&desktopLink('/bookmarks',FiBookmark,'Bookmarks')}{desktopLink(user?'/profile':'/auth',FiUser,user?'Profile':'Sign In')}</nav><Link to={user?'/create':'/auth'} onClick={()=>setSidebarOpen(false)} className="mt-6 block rounded-full bg-[#3B82F6] px-5 py-3.5 text-center text-sm font-bold">{user?'Post':'Sign In'}</Link></aside></div>}
