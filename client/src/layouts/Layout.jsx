...
const navItems = [
  ...
  { name: 'Sermons', path: '/rhapsody', icon: FiMic },
...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: 'radial-gradient(ellipse at top left, rgba(120,20,60,0.45) 0%, transparent 45%), radial-gradient(ellipse at bottom right, rgba(60,20,100,0.35) 0%, transparent 45%), #0d0c18' }}>
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.07]" style={{ background: 'rgba(13,12,24,0.92)', backdropFilter: 'blur(20px)' }}>
        <div className="flex items-center gap-3 px-4 py-2.5 sm:px-5">
          ...
          <Link to="/" className="flex shrink-0 items-center gap-3">
            <img src="/logo.png" alt="BLW Logo" className="h-9 w-9 rounded-full object-cover ring-1 ring-white/10" />
            <span className="hidden sm:block">
              <span className="header-brand block text-white leading-tight whitespace-nowrap" style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 800, fontSize: '0.95rem' }}>
                Believers' LoveWorld CM
              </span>
              <span className="block whitespace-nowrap" style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 600, fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '6px', background: 'linear-gradient(90deg,#F2A31C,#F6C94E)', WebkitBackgroundClip: 'text', color: 'transparent' }}>
                East &amp; Central Africa
              </span>
            </span>
          </Link>
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            {user ? (
              ...
              >
                <FiUser className="h-3.5 w-3.5" /> My Account
              </Link>
            ) : (
              <Link
                to="/auth"
                ...
                style={{ background: 'linear-gradient(135deg,#EC2FA8,#8A2BE2)' }}
              >
                <FiLogIn className="h-3.5 w-3.5" /> Sign In
              </Link>
            )}
            <button
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="border-t border-white/[0.07] px-4 py-4 sm:hidden" style={{ background: 'rgba(13,12,24,0.97)' }}>
      ...
      {/* ── FOOTER ── */}
      <footer className="border-t border-white/[0.07] mt-8" style={{ background: 'rgba(10,9,20,0.8)' }}>
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-10 sm:grid-cols-3">
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              <img src="/logo.png" alt="BLW Logo" className="h-8 w-8 rounded-full object-cover ring-1 ring-white/10" />
              <span className="font-semibold text-white text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>Believers' LoveWorld CM</span>
            </div>
            <p className="text-xs text-white/40 leading-relaxed">Fellowship Without Borders for students and young professionals across East &amp; Central Africa.</p>
          </div>
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-widest" style={{ color: '#F2A31C' }}>Quick Links</h4>
          ...
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-widest" style={{ color: '#F2A31C' }}>Stay Connected</h4>
            <div className="flex items-center gap-3 text-white/50">
              <a href="https://wa.me/254700000000" className="rounded-xl border border-white/10 p-3 transition hover:border-white/25 hover:text-white" aria-label="WhatsApp"><FiPhone className="h-4 w-4" /></a>
              <a href="/connect" className="rounded-xl border border-white/10 p-3 transition hover:border-white/25 hover:text-white" aria-label="Connect"><FiHeart className="h-4 w-4" /></a>
            </div>
          </div>
        </div>
