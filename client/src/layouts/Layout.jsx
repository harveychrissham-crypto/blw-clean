...
export default function Layout({ children }) {
  ...
  return <div className="min-h-screen text-white" style={{background:'#0B0F14'}}>
    ...
    {sidebarOpen&&<div className="fixed inset-0 z-[70] hidden lg:block"><div className="absolute inset-0 bg-black/60" onClick={()=>setSidebarOpen(false)}/><aside className="absolute inset-y-0 left-0 w-[360px] max-w-[88vw] border-r border-white/[0.07] bg-[#0B0F14] flex flex-col px-0 py-4 shadow-2xl"><div className="flex shrink-0 items-center px-5 mb-6"><Link to="/" onClick={()=>setSidebarOpen(false)} aria-label="Emet home"><img src="/emet-wordmark-white.svg" alt="Emet" className="h-12 w-auto"/></Link></div><nav className="min-h-0 flex-1 overflow-y-auto flex flex-col gap-0.5 px-0 pb-3">{navItems.map(item=>{const Icon=item.icon;return <NavLink key={item.path} to={item.path} end={item.path==='/'} onClick={()=>setSidebarOpen(false)} className={({isActive})=>`flex items-center gap-5 rounded-r-xl px-5 py-3.5 text-base font-semibold ${isActive?'bg-white/[.09] text-white':'text-white/55 hover:bg-white/[.05] hover:text-white'}`}><Icon className="h-6 w-6 shrink-0"/>{item.name}</NavLink>;})}{user&&desktopLink('/notifications',FiBell,'Notifications')}{user&&desktopLink('/messages',FiMessageCircle,'Messages')}{user&&desktopLink('/bookmarks',FiBookmark,'Bookmarks')}{desktopLink(user?'/profile':'/auth',FiUser,user?'Profile':'Sign In')}</nav><Link to={user?'/create':'/auth'} onClick={()=>setSidebarOpen(false)} className="mt-auto shrink-0 mx-4 rounded-xl bg-[#1D9BF0] px-5 py-3 text-center text-sm font-semibold shadow-sm hover:bg-[#1A8CD8]">{user?'Post':'Sign In'}</Link></aside></div>}
