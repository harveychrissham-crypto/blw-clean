...
export default function FellowshipLocationsAdmin() {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8"><div className="mb-8 flex items-start justify-between gap-4"><div><Eyebrow color="#D8B2FF">Leaders tool</Eyebrow><h1 className="mt-2 text-3xl font-bold text-white sm:text-4xl">Manage Fellowship Locations</h1><p className="mt-3 max-w-3xl text-sm leading-relaxed text-white/45">Search a town or university, click the exact fellowship location, drag the red pin to fine-tune it, and save the fellowship details. Saved fellowships appear as red pins.</p></div><button onClick={() => { sessionStorage.removeItem('blw_leader_admin_token'); setToken(''); }} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiX /> Close session</button></div><div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]"><Card as="form" onSubmit={saveLocation} variant="raised" className="p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><Eyebrow>{editingId ? 'Edit fellowship' : 'Add fellowship'}</Eyebrow><h2 className="mt-1 text-xl font-bold text-white">Place the pin on the map</h2></div>{editingId && <button type="button" onClick={resetForm} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2 text-xs font-semibold text-white/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiArrowLeft /> New location</button>}</div><div className="mb-4 flex flex-col gap-2 sm:flex-row"><input value={mapQuery} onChange={e => setMapQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); searchMapPlace(); } }} placeholder="Search Juja, Thika, Ruiru, JKUAT…" className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none" /><button type="button" onClick={searchMapPlace} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white/[0.07] px-5 py-3 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiSearch /> Find place</button></div><div className="overflow-hidden rounded-2xl border border-white/10 bg-black/20"><PinMap value={form} locations={locations} onChange={updatePin} /></div><div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2"><p className="text-[9px] uppercase tracking-[0.2em] text-white/30">Latitude</p><p className="mt-1 text-xs text-white/70">{form.latitude || 'Click the map'}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2"><p className="text-[9px] uppercase tracking-[0.2em] text-white/30">Longitude</p><p className="mt-1 text-xs text-white/70">{form.longitude || 'Click the map'}</p></div></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="sm:col-span-2"><span className="mb-1 block text-xs font-semibold text-white/55">Fellowship Name *</span><input required value={form.fellowshipName} onChange={setField('fellowshipName')} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white" /></label><label><span className="mb-1 block text-xs font-semibold text-white/55">University</span><input value={form.university} onChange={setField('university')} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white" /></label></div><div className="mt-5 border-t border-white/[0.06] pt-5"><p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/40">Location</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label><span className="mb-1 block text-xs font-semibold text-white/55">Town / Area</span><input value={form.town} onChange={setField('town')} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white" /></label><label><span className="mb-1 block text-xs font-semibold text-white/55">Address / Landmark</span><input value={form.address} onChange={setField('address')} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white" /></label></div></div><div className="mt-5 border-t border-white/[0.06] pt-5"><p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/40">Schedule &amp; details</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label><span className="mb-1 block text-xs font-semibold text-white/55">Service Time</span><input value={form.serviceTime} onChange={setField('serviceTime')} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white" /></label><label className="sm:col-span-2"><span className="mb-1 block text-xs font-semibold text-white/55">Description</span><textarea value={form.description} onChange={setField('description')} rows={3} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white" /></label></div></div>{error && <p className="mt-5 rounded-xl bg-red-500/10 px-4 py-3 text-sm text-red-300">{error}</p>}{message && <p className="mt-5 rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">{message}</p>}<button disabled={saving} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#EC2FA8] via-[#8A2BE2] to-[#3D5AFE] py-3.5 text-sm font-bold text-white disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40">{saving ? <FiLoader className="animate-spin" /> : <FiCheck />}{saving ? 'Saving…' : editingId ? 'Save Changes' : 'Save Fellowship'}</button></Card><Card as="aside" variant="raised" className="p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><Eyebrow>Saved locations</Eyebrow><h2 className="mt-1 text-xl font-bold text-white">Fellowship pins</h2></div>{loading && <FiLoader className="animate-spin text-white/40" />}</div><div className="space-y-3">{loading && !locations.length ? <SkeletonList rows={3} /> : locations.map(l => <Card key={l.id} variant="subtle" className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-white">{l.fellowshipName}</p><p className="mt-1 text-xs text-white/45">{[l.university, l.town || l.area, l.serviceTime].filter(Boolean).join(' • ')}</p><p className="mt-2 text-[10px] text-white/25">{Number(l.latitude).toFixed(6)}, {Number(l.longitude).toFixed(6)}</p></div><div className="flex gap-1"><button type="button" onClick={() => editLocation(l)} className="rounded-lg p-2 text-white/50 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiEdit2 /></button><button type="button" onClick={() => deleteLocation(l)} className="rounded-lg p-2 text-white/50 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiTrash2 /></button></div></div></Card>)}{!locations.length && !loading && <EmptyState icon={FiPlus} title="No fellowship locations yet" hint="Saved fellowships will appear here as red pins." />}</div></Card></div></section>;
}
