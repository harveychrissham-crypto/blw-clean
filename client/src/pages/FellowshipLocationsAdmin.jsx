...
function addSavedMarker(L, map, location) {
  ...
  const icon = L.divIcon({
    className: 'blw-saved-fellowship-marker',
    html: '<div style="width:26px;height:26px;background:#dc2626;border:3px solid #fff;border-radius:50% 50% 50% 0;transform:rotate(-45deg);box-shadow:0 2px 8px rgba(0,0,0,.45);position:relative"><span style="position:absolute;inset:5px;border-radius:50%;background:#fff"></span></div>',
    iconSize: [26, 26],
    iconAnchor: [13, 26],
    popupAnchor: [0, -26],
  });
...
function PinMap({ value, locations, onChange }) {
  ...
  useEffect(() => {
    ...
    if (!markerRef.current) addEditableMarker(L, map, [lat, lng], onChange, markerRef);
...
export default function FellowshipLocationsAdmin() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !body.token) throw new Error(body.error || 'Leadership session could not be established.');
        if (!cancelled) { sessionStorage.setItem('blw_leader_admin_token', body.token); setToken(body.token); }
      } catch (err) {
        if (!cancelled) setError(err.message || 'Leadership session could not be established.');
      } finally {
  ...
  const loadLocations = async () => {
    ...
    setLoading(true);
    try {
      const r = await authedFetch('/api/fellowships/admin');
      const b = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(b.error || 'Unable to load fellowship locations.');
      setLocations(Array.isArray(b.fellowships) ? b.fellowships : []);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const searchMapPlace = async () => {
    ...
    try {
      ...
      const b = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(b.error || 'Unable to search for that place.');
      const result = b.results?.[0];
      if (!result) throw new Error(`No map result found for “${q}”.`);
      updatePin({ latitude: Number(result.lat).toFixed(6), longitude: Number(result.lon).toFixed(6) });
      setMessage(`Map centered on ${result.displayName || q}. Click the map or drag the pin to the exact fellowship.`);
    } catch (e) {
      setError(e.message);
      setMessage('');
    }
  ...
  const saveLocation = async e => {
    ...
    try {
      ...
      if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) throw new Error('Place a pin on the map before saving.');
      const payload = { ...form, latitude, longitude, country: form.country || 'Kenya' };
      const r = await authedFetch(editingId ? `/api/fellowships/admin/${editingId}` : '/api/fellowships/admin', { method: editingId ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const b = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(b.error || 'Unable to save fellowship location.');
      const saved = b.fellowship;
      if (saved) setLocations(current => editingId ? current.map(item => item.id === saved.id ? saved : item) : [...current, saved]);
      setMessage(`Fellowship saved at ${latitude.toFixed(6)}, ${longitude.toFixed(6)}.`);
      resetForm();
      await loadLocations();
    } catch (err) {
  ...
  const editLocation = l => { setEditingId(l.id); setForm({ ...emptyForm, ...l, latitude: String(l.latitude ?? ''), longitude: String(l.longitude ?? '') }); setMessage('Edit mode: move the pin or update the details, then save.'); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const deleteLocation = async l => {
    if (!window.confirm(`Delete “${l.fellowshipName}”?`)) return;
    try {
      const r = await authedFetch(`/api/fellowships/admin/${l.id}`, { method: 'DELETE' });
      const b = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(b.error || 'Unable to delete fellowship location.');
