      ...
      {showCreate && <div className="fixed inset-0 z-[90] grid place-items-center bg-black/70 p-4" onClick={() => setShowCreate(false)}>
        <form onSubmit={submitCreate} onClick={(event) => event.stopPropagation()} className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#11161D] p-6 shadow-2xl">
          ...
          <div className="mt-6 space-y-4">
            <input required maxLength={80} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Community name" className="w-full rounded-2xl border border-white/12 bg-[#11161D] px-4 py-3 text-sm outline-none focus:border-[#1D9BF0]" />
            <textarea required maxLength={500} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="What is this community about?" rows={4} className="w-full resize-none rounded-2xl border border-white/12 bg-[#11161D] px-4 py-3 text-sm outline-none focus:border-[#1D9BF0]" />
