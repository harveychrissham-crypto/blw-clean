...
import { FiAlertCircle, FiBookmark, FiHeart, FiLoader, FiMessageCircle, FiMoreHorizontal, FiSend, FiShare2, FiRepeat, FiMessageSquare, FiVolume2, FiVolumeX, FiWifiOff, FiX, FiFlag, FiLink2, FiTrash2 } from 'react-icons/fi';
import { useAuth } from '../context/AuthContext';
import { useOnlineStatus } from '../hooks/useOnlineStatus';
import { fetchFeed, toggleLike, toggleSave, toggleRepost, createQuote, toggleFollow, fetchComments, addComment, deleteComment, reportPost, deletePost, readCachedFeed, writeCachedFeed, recordFeedView } from '../utils/feed';
...
function formatLikeLine(post) {
  ...
  const likers = (post.recentLikers || []).filter(Boolean);
  const names = likers.map((l) => (typeof l === 'string' ? l : l?.name)).filter(Boolean);
...
function LikeAvatars({ post }) {
  const likers = (post.recentLikers || [])
    .filter(Boolean)
    .map((l) => (typeof l === 'string' ? { name: l, avatarUrl: '' } : l))
    .filter((l) => l?.name)
    .slice(0, 2);
  if (!likers.length) return null;
  return <div className="mr-1.5 flex -space-x-2">{likers.map((l, i) => <div key={i} className="h-4 w-4 overflow-hidden rounded-full bg-white/10 ring-2 ring-[#0B0F14]">{l.avatarUrl ? <img src={l.avatarUrl} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full w-full place-items-center text-[7px] font-bold text-white/60">{l.name.charAt(0).toUpperCase()}</div>}</div>)}</div>;
}

const REPORT_REASONS = ['Spam', 'Inappropriate', 'Harassment', 'False information', 'Other'];

function ActionMenu({post,onRemove,dark}) {
  const [open,setOpen]=useState(false); const [reporting,setReporting]=useState(false); const [reported,setReported]=useState(false); const [confirmingDelete,setConfirmingDelete]=useState(false); const [deleting,setDeleting]=useState(false); const [deleteError,setDeleteError]=useState(''); const menuRef=useRef(null);
  ...
  const confirmDelete=async()=>{setDeleting(true);setDeleteError('');try{await deletePost(post.id);hapticSuccess();setOpen(false);onRemove?.(post.id);}catch(e){setDeleteError(e?.message||'Unable to delete this post right now.');hapticError();}finally{setDeleting(false);}};
  return <div ref={menuRef} className="relative"><button type="button" onClick={()=>setOpen(v=>!v)} className={dark?'grid h-9 w-9 place-items-center rounded-full bg-black/35 text-white backdrop-blur-md hover:bg-black/55':'grid h-9 w-9 place-items-center rounded-full text-white/45 hover:bg-white/[.06]'} aria-label="More options"><FiMoreHorizontal/></button><AnimatePresence>{open&&<motion.div initial={{opacity:0,scale:.96,y:-4}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.96,y:-4}} className="absolute right-0 top-10 z-30 min-w-48 overflow-hidden rounded-xl border border-white/10 bg-[#0B0F14]/95 p-1 shadow-2xl backdrop-blur-xl">
    {reported ? <div className="px-3 py-3 text-center text-xs text-white/70">Thanks — this has been reported.</div>
    : confirmingDelete ? <div className="p-2"><p className="px-1 pb-2 text-xs text-white/70">Delete this post? This can't be undone.</p>{deleteError&&<p className="px-1 pb-2 text-xs text-red-300">{deleteError}</p>}<div className="flex gap-2"><button type="button" onClick={()=>{setConfirmingDelete(false);setDeleteError('');}} disabled={deleting} className="flex-1 rounded-lg bg-white/[.06] px-2 py-2 text-xs font-semibold text-white/80 hover:bg-white/[.1] disabled:opacity-50">Cancel</button><button type="button" onClick={confirmDelete} disabled={deleting} className="flex-1 rounded-lg bg-red-500/20 px-2 py-2 text-xs font-bold text-red-300 hover:bg-red-500/30 disabled:opacity-50">{deleting?'Deleting…':'Delete'}</button></div></div>
    ...
    : <><button type="button" onClick={copy} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-white/80 hover:bg-white/[.06]"><FiLink2/>Copy link</button>{post.isOwner?<button type="button" onClick={()=>{setConfirmingDelete(true);hapticTap();}} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-red-300 hover:bg-red-500/10"><FiTrash2/>Delete post</button>:<button type="button" onClick={()=>{setReporting(true);hapticTap();}} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-white/80 hover:bg-white/[.06]"><FiFlag/>Report</button>}</>}
...
function Actions({post,user,onUpdate,onComments,onQuote}) {
  const [busy,setBusy]=useState('');
  const action=async name=>{if(!user)return hapticError();setBusy(name);const previous={liked:post.liked,saved:post.saved,reposted:post.reposted,likeCount:post.likeCount,saveCount:post.saveCount,repostCount:post.repostCount};const patch=name==='like'?{liked:!post.liked,likeCount:Math.max(0,post.likeCount+(post.liked?-1:1))}:name==='save'?{saved:!post.saved,saveCount:Math.max(0,post.saveCount+(post.saved?-1:1))}:{reposted:!post.reposted,repostCount:Math.max(0,post.repostCount+(post.reposted?-1:1))};onUpdate(post.id,patch);try{const r=name==='like'?await toggleLike(post.id):name==='save'?await toggleSave(post.id):await toggleRepost(post.id);onUpdate(post.id,r);hapticSuccess();}catch{onUpdate(post.id,previous);hapticError();}finally{setBusy('');}};
  ...
  return <div className="flex items-center gap-1 pt-3"><button aria-label={post.liked?'Unlike':'Like'} disabled={busy==='like'} onClick={()=>action('like')} className={`grid h-9 w-9 place-items-center rounded-full transition hover:bg-white/[.06] active:scale-90 ${post.liked?'text-[#3B82F6]':'text-white/75'}`}><FiHeart className="h-6 w-6" fill={post.liked?'currentColor':'none'}/></button><button aria-label={post.reposted?'Undo repost':'Repost'} disabled={busy==='repost'} onClick={()=>action('repost')} className={`grid h-9 w-9 place-items-center rounded-full transition hover:bg-white/[.06] active:scale-90 ${post.reposted?'text-emerald-300':'text-white/75'}`}><FiRepeat className="h-6 w-6"/></button><span className="text-xs text-white/35">{post.repostCount||0}</span><button aria-label="Quote post" onClick={()=>onQuote?.(post)} className="grid h-9 w-9 place-items-center rounded-full text-white/75 transition hover:bg-white/[.06] active:scale-90"><FiMessageSquare className="h-5 w-5"/></button><button aria-label="Comment" onClick={onComments} className="grid h-9 w-9 place-items-center rounded-full text-white/75 transition hover:bg-white/[.06] active:scale-90"><FiMessageCircle className="h-6 w-6"/></button><button aria-label="Share" onClick={share} className="grid h-9 w-9 place-items-center rounded-full text-white/75 transition hover:bg-white/[.06] active:scale-90"><FiSend className="h-6 w-6"/></button><button aria-label={post.saved?'Unsave':'Save'} disabled={busy==='save'} onClick={()=>action('save')} className={`ml-auto grid h-9 w-9 place-items-center rounded-full transition hover:bg-white/[.06] active:scale-90 ${post.saved?'text-white':'text-white/65'}`}><FiBookmark className="h-6 w-6" fill={post.saved?'currentColor':'none'}/></button></div>;
}
...
function VideoMedia({post,active=false,reel=false,onDoubleTap}) {
  ...
  return <div className="relative h-full w-full" onDoubleClick={onDoubleTap}>{media}{!isYoutube&&<div className="pointer-events-none absolute inset-x-0 bottom-0 h-1 bg-white/10"><div className="h-full bg-white/80" style={{width:`${progress}%`}}/></div>}{(reel||!isYoutube)&&<button type="button" onClick={toggleMute} className="absolute bottom-4 right-4 z-10 rounded-full bg-black/55 p-2.5 text-white backdrop-blur-xl transition hover:bg-black/70 active:scale-90" aria-label={muted?'Unmute video':'Mute video'}>{muted?<FiVolumeX/>:<FiVolume2/>}</button>}</div>;
}
...
function PostCard({post,user,onUpdate,onRemove,onFollowUpdate,onQuote,notificationId}) {
  ...
  const follow=async()=>{if(!user||!post.authorEmail||post.isOwner||followBusy)return;setFollowBusy(true);const next=!post.following;const patchAuthor=onFollowUpdate||((email,patch)=>onUpdate(post.id,patch));patchAuthor(post.authorEmail,{following:next});try{const r=await toggleFollow(post.authorEmail);patchAuthor(post.authorEmail,r);hapticSuccess();}catch{patchAuthor(post.authorEmail,{following:!next});hapticError();}finally{setFollowBusy(false);}};
...
function ReelCard({post,user,onUpdate,onRemove,active,notificationId}) {
  ...
  const doubleTap=useDoubleTap(like);
  return <article id={`feed-${post.id}`} className={`relative h-full min-h-[540px] snap-start overflow-hidden bg-black ${String(post.id)===String(notificationId)?'ring-2 ring-white':''}`}><div className="absolute inset-0" onDoubleClick={doubleTap}><>{post.videoId||post.mediaUrl ? (post.mediaType?.startsWith?.('image/') ? <img src={post.mediaUrl} alt={post.title||''} className="h-full w-full object-cover" loading="lazy" /> : <VideoMedia post={post} active={active} reel onDoubleTap={doubleTap}/>) : <div className="flex h-full w-full items-center justify-center bg-gradient-to-b from-[#0B0F14] via-[#0B0F14] to-black p-8 text-center"><div className="max-w-xl"><p className="text-sm font-bold text-white/55">{post.author}</p><h2 className="mt-4 text-3xl font-black leading-tight text-white sm:text-5xl">{post.title}</h2>{post.body&&post.body!==post.title&&<p className="mt-4 whitespace-pre-wrap text-base leading-relaxed text-white/65 sm:text-lg">{post.body}</p>}</div></div>}</></div><AnimatePresence>{heart&&<motion.div initial={{opacity:0,scale:.4}} animate={{opacity:1,scale:1.1}} exit={{opacity:0,scale:1.35}} className="pointer-events-none absolute inset-0 z-20 grid place-items-center"><FiHeart className="h-28 w-28 text-white drop-shadow-2xl" fill="currentColor"/></motion.div>}</AnimatePresence><div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/25 via-transparent to-black/90"/><div className="absolute right-3 z-10" style={{top:'max(0.75rem, env(safe-area-inset-top))'}}><ActionMenu post={post} onRemove={onRemove} dark/></div><div className="absolute bottom-0 left-0 right-0 z-10 p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]"><div className="flex items-end gap-5"><div className="min-w-0 flex-1"><p className="text-sm font-bold">{post.author}</p><h2 className="mt-2 text-lg font-bold">{post.title}</h2>{post.body&&<p className="mt-1 line-clamp-2 text-sm text-white/75">{post.body}</p>}{showViews&&<p className="mt-2 text-xs text-white/50">{post.viewCount} views</p>}</div><div className="mr-2 flex flex-col items-center gap-3">{[[post.liked,FiHeart,like,post.likeCount,post.liked?'Unlike':'Like',post.liked],[false,FiMessageCircle,()=>setComments(true),post.commentCount,'Comment',false],[false,FiShare2,share,null,'Share',false],[post.saved,FiBookmark,save,null,post.saved?'Unsave':'Save',false]].map(([on,Icon,fn,count,label,tinted])=><button key={label} type="button" onClick={fn} disabled={busyAction!==''} aria-label={label} className={`grid min-w-10 place-items-center rounded-full bg-black/35 p-2.5 backdrop-blur-md transition hover:bg-black/55 disabled:opacity-50 ${tinted?'text-[#3B82F6]':'text-white'}`}><Icon className="h-6 w-6" fill={on?'currentColor':'none'}/>{count!=null&&<span className="text-[10px]">{count}</span>}</button>)}</div></div></div><Comments post={post} user={user} onUpdate={onUpdate} open={comments} onClose={()=>setComments(false)}/></article>;
}

function Reels({posts,user,onUpdate,onRemove,notificationId}) {
  ...
  const reelIds=posts.map(p=>p.id).join(',');
  useEffect(()=>{const root=ref.current;if(!root)return;const cards=[...root.querySelectorAll('[data-reel]')];const obs=new IntersectionObserver(entries=>{const e=entries.filter(x=>x.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(e)setActive(e.target.dataset.reel);},{root,threshold:.6});cards.forEach(c=>obs.observe(c));return()=>obs.disconnect();},[reelIds]);
  ...
  return <div ref={ref} className="h-[calc(100dvh-8.5rem)] snap-y snap-mandatory overflow-y-auto" style={{scrollbarWidth:'none'}}>{posts.map(p=><div key={p.id} data-reel={p.id} className="h-[calc(100dvh-8.5rem)]"><ReelCard post={p} user={user} onUpdate={onUpdate} onRemove={onRemove} active={active===String(p.id)} notificationId={notificationId}/></div>)}</div>;
}
...
function Comments({post,user,onUpdate,open,onClose}) {
  ...
  const submit=async e=>{e.preventDefault();if(!user||!text.trim())return;const body=text.trim();setText('');try{const created=await addComment(post.id,body);setItems(v=>[...v,created]);onUpdate(post.id,{commentCount:post.commentCount+1});hapticSuccess();}catch{setText(body);hapticError();}};
  ...
  if(!open)return null;
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/60 p-0 sm:p-6" onClick={onClose}><motion.div initial={{y:'100%'}} animate={{y:0}} onClick={e=>e.stopPropagation()} className="flex h-[78dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-[28px] border border-white/10 bg-[#0B0F14] shadow-2xl sm:rounded-[28px]"><div className="flex justify-center pt-3"><span className="h-1 w-12 rounded-full bg-white/20"/></div><div className="flex items-center justify-between border-b border-white/10 px-4 py-3"><h3 className="font-bold">Comments</h3><button type="button" onClick={onClose} aria-label="Close comments" className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/[.06]"><FiX/></button></div><div ref={listRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">{loading?<div className="flex items-center justify-center py-10 text-white/50"><FiLoader className="animate-spin"/></div>:items.length?items.map(c=><div key={c.id} className="flex items-start gap-3"><div className="h-8 w-8 shrink-0 overflow-hidden rounded-full bg-white/[.08]"><img src={c.avatarUrl||'/logo.png'} alt="" className="h-full w-full object-cover" loading="lazy" decoding="async"/></div><div className="min-w-0 flex-1"><p className="text-sm font-semibold">{c.author}</p><p className="text-sm text-white/70">{c.body}</p></div>{user&&(post.isOwner||c.user_email&&c.user_email.toLowerCase()===user.email?.toLowerCase())&&<button type="button" onClick={()=>remove(c.id)} disabled={removingId===c.id} aria-label="Delete comment" className="shrink-0 rounded-full p-1.5 text-white/30 hover:bg-white/[.06] hover:text-red-300 disabled:opacity-40"><FiTrash2 className="h-3.5 w-3.5"/></button>}</div>):<EmptyState title="No comments yet" description="Be the first to add a comment."/>}</div>{user&&<form onSubmit={submit} className="flex gap-2 border-t border-white/10 p-3"><input value={text} onChange={e=>setText(e.target.value)} placeholder="Add a comment..." className="min-w-0 flex-1 rounded-full bg-white/[.06] px-4 py-3 text-sm outline-none"/><button className="rounded-full bg-white px-4 py-2 text-sm font-bold text-black transition hover:bg-white/90 disabled:opacity-50" disabled={!text.trim()}>Post</button></form>}</motion.div></div>;
}

function QuoteModal({post,body,setBody,onClose,onSubmit,busy,error}) {
  return <AnimatePresence>{post&&<motion.div className="fixed inset-0 z-[100] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    <motion.div initial={{opacity:0,y:12,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:12,scale:.98}} className="w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-[#0B0F14] shadow-2xl">
      <div className="flex items-center justify-between border-b border-white/[.07] px-4 py-3"><h2 className="text-sm font-bold">Quote post</h2><button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-full text-white/50 hover:bg-white/[.06]" aria-label="Close"><FiX/></button></div>
      <div className="p-4"><textarea autoFocus value={body} onChange={e=>setBody(e.target.value)} maxLength={5000} placeholder="Add your thoughts…" className="min-h-28 w-full resize-none rounded-xl border border-white/10 bg-white/[.04] p-3 text-sm text-white outline-none placeholder:text-white/30 focus:border-white/20"/>
        <div className="mt-4 rounded-xl border border-white/10 bg-white/[.025] p-3"><p className="text-xs font-bold text-white/80">{post.author}</p><p className="mt-1 line-clamp-3 whitespace-pre-wrap text-sm text-white/55">{post.body||post.title}</p></div>
        {error&&<p className="mt-3 text-xs text-red-300">{error}</p>}
        <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onClose} disabled={busy} className="rounded-full px-4 py-2 text-xs font-bold text-white/60 hover:bg-white/[.06]">Cancel</button><button type="button" onClick={onSubmit} disabled={busy||!body.trim()} className="rounded-full bg-white px-5 py-2 text-xs font-bold text-black disabled:opacity-40">{busy?'Posting…':'Quote'}</button></div>
      </div>
    </motion.div>
...
export default function Feed(){
  ...
  // those posts should update all of them, not just the post that was
  // tapped (update() above only ever patches a single post by id).
  const updateAuthor=useCallback((authorEmail,patch)=>{const email=String(authorEmail||'').toLowerCase();if(!email)return;setPosts(v=>v.map(p=>p.authorEmail&&p.authorEmail.toLowerCase()===email?{...p,...patch}:p));},[]);
  const remove=useCallback((id)=>{setPosts(v=>v.filter(p=>String(p.id)!==String(id)));},[]);
  const openQuote=useCallback((post)=>{if(!user){hapticError();return;}setQuoteTarget(post);setQuoteBody('');setQuoteError('');},[user]);
  const submitQuote=useCallback(async()=>{if(!quoteTarget||!user||quoteBusy)return;setQuoteBusy(true);setQuoteError('');try{const created=await createQuote(quoteTarget.id,quoteBody.trim());setPosts(v=>[created,...v]);setQuoteTarget(null);setQuoteBody('');hapticSuccess();}catch(e){setQuoteError(e?.message||'Unable to publish your quote.');hapticError();}finally{setQuoteBusy(false);}},[quoteTarget,user,quoteBusy,quoteBody]);
  useEffect(()=>()=>cancelAnimationFrame(updateFrame.current),[]);
  const loadFirst=useCallback(async({silent=false}={})=>{if(!silent)setLoading(true);setError('');try{const result=await fetchFeed({limit:PAGE_SIZE,offset:0,followingOnly:tab==='Following',feedType:tab==='Reels'?'reel':tab==='Ministry'?'ministry':''});const normalized=result.posts.map(normalize);if(silent){setPosts(current=>{const currentIds=new Set(current.map(p=>String(p.id)));const incoming=normalized.filter(p=>!currentIds.has(String(p.id)));if(incoming.length)setNewPosts(v=>v+incoming.length);return incoming.length?[...incoming,...current]:current;});}else{setPosts(normalized);setHasMore(result.hasMore);if(tab==='All')writeCachedFeed(normalized);}}catch(e){if(!silent)setError(e?.message||'Unable to load the Feed.');}finally{if(!silent)setLoading(false);}},[tab]);
  const loadMore=useCallback(async()=>{if(loading||loadingMore||!hasMore||messagesOpen)return;setLoadingMore(true);try{const result=await fetchFeed({limit:PAGE_SIZE,offset:posts.length,followingOnly:tab==='Following',feedType:tab==='Reels'?'reel':tab==='Ministry'?'ministry':''});const more=result.posts.map((p,i)=>normalize(p,posts.length+i));setPosts(v=>{const ids=new Set(v.map(x=>String(x.id)));return [...v,...more.filter(x=>!ids.has(String(x.id)))];});setHasMore(result.hasMore);}catch{}finally{setLoadingMore(false);}},[loading,loadingMore,hasMore,messagesOpen,posts.length,tab]);
  ...
  useEffect(()=>{if(messagesOpen||tab==='Reels')return;const id=setInterval(()=>{if(document.visibilityState==='visible')loadFirst({silent:true});},45000);return()=>clearInterval(id);},[loadFirst,messagesOpen,tab]);
  ...
  // which could leave Reels/Ministry looking sparse or empty until enough
  // "load more" pages had been fetched to surface matching items.
  if(messagesOpen)return <Messages/>;
  return <main {...bind} className="min-h-screen bg-[#0B0F14] pb-24 text-white"><div className="sticky top-0 z-40 border-b border-white/[.06] bg-[#0B0F14]/90 px-4 py-3 backdrop-blur-xl"><div className="mx-auto flex max-w-2xl items-center justify-between"><Link to="/dashboard" className="h-9 w-9 overflow-hidden rounded-full bg-white/[.07] transition hover:opacity-80" aria-label="Profile"><img src={user?.avatar_url||user?.avatarUrl||'/logo.png'} alt="" className="h-full w-full object-cover" loading="lazy" decoding="async"/></Link><div className="flex items-center gap-2"><Link to="/feed?messages=1" aria-label="Open messages" className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/[.06]"><FiMessageCircle/></Link><Link to="/create" className="hidden rounded-full bg-white px-4 py-2 text-xs font-bold text-black transition hover:bg-white/90 sm:inline-flex">Create</Link></div></div></div><div className="mx-auto max-w-2xl"><StoriesRow/><div className="sticky top-[61px] z-30 border-b border-white/[.06] bg-[#0B0F14]/90 px-4 backdrop-blur-xl"><div className="flex gap-5 overflow-x-auto py-3">{tabs.map(t=><button key={t} type="button" onClick={()=>setTab(t)} className={`relative whitespace-nowrap pb-1.5 text-xs font-bold transition ${tab===t?'text-white':'text-white/40 hover:text-white/65'}`}>{t}{tab===t&&<motion.span layoutId="feed-tab-indicator" className="absolute -bottom-[1px] left-1/2 h-[3px] w-5 -translate-x-1/2 rounded-full bg-white" transition={{type:'spring',stiffness:500,damping:35}}/>}</button>)}</div></div>{newPosts>0&&<button type="button" onClick={()=>{setNewPosts(0);window.scrollTo({top:0,behavior:'smooth'});loadFirst();}} className="sticky top-[104px] z-20 mx-auto mt-3 block rounded-full bg-white px-4 py-2 text-xs font-bold text-black shadow-lg transition hover:bg-white/90 active:scale-95">{newPosts} new post{newPosts===1?'':'s'}</button>}{error&&<div className="m-4 rounded-xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200"><div className="flex items-center gap-2"><FiAlertCircle/>{error}</div></div>}{!online&&<div className="m-4 flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] p-3 text-xs text-white/60"><FiWifiOff/>You're offline. Showing cached content when available.</div>}{loading&&!posts.length?<div className="h-[calc(100dvh-8.5rem)] p-4"><Skeleton className="h-full w-full rounded-2xl"/></div>:posts.length?<><Reels posts={posts} user={user} onUpdate={update} onRemove={remove} notificationId={notificationId}/><div ref={sentinel} className="flex justify-center py-8">{loadingMore&&<FiLoader className="animate-spin text-white/40"/>}</div></>:<EmptyState title="Nothing here yet" description="New posts and updates from the Emet community will appear in your Feed."/>}</div><QuoteModal post={quoteTarget} body={quoteBody} setBody={setQuoteBody} onClose={()=>{if(!quoteBusy)setQuoteTarget(null);}} onSubmit={submitQuote} busy={quoteBusy} error={quoteError}/></main>;
}
