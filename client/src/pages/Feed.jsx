...
function Reels({posts,user,onUpdate,onRemove,notificationId}) {
  ...
  return <div ref={ref} className="h-full snap-y snap-mandatory overflow-y-auto" style={{scrollbarWidth:'none'}}>{posts.map(p=><div key={p.id} data-reel={p.id} className="h-full"><ReelCard post={p} user={user} onUpdate={onUpdate} onRemove={onRemove} active={active===String(p.id)} notificationId={notificationId}/></div>)}</div>;
}
...
export default function Feed(){
  ...
  return <main {...bind} className="fixed inset-0 overflow-hidden bg-black text-white"><div className="absolute inset-0 z-50 pointer-events-none"><div className="pointer-events-auto absolute left-4 top-4 flex items-center gap-3"><Link to="/dashboard" className="h-10 w-10 overflow-hidden rounded-full border border-white/20 bg-black/40 backdrop-blur-xl" aria-label="Profile"><img src={user?.avatar_url||user?.avatarUrl||'/logo.png'} alt="" className="h-full w-full object-cover"/></Link><span className="hidden text-sm font-bold sm:block">Feed</span></div><div className="pointer-events-auto absolute right-4 top-4 flex items-center gap-2"><Link to="/feed?messages=1" aria-label="Open messages" className="grid h-10 w-10 place-items-center rounded-full bg-black/40 text-white backdrop-blur-xl hover:bg-black/60"><FiMessageCircle/></Link><Link to="/create" aria-label="Create" className="grid h-10 w-10 place-items-center rounded-full bg-white text-black backdrop-blur-xl hover:bg-white/90"><span className="text-lg font-bold">+</span></Link></div></div>{error&&<div className="absolute left-1/2 top-20 z-[60] w-[min(92vw,520px)] -translate-x-1/2 rounded-xl border border-red-400/20 bg-black/75 p-3 text-sm text-red-200 backdrop-blur-xl"><FiAlertCircle className="mr-2 inline"/>{error}</div>}{loading&&!posts.length?<div className="h-full w-full animate-pulse bg-[#0B0F14]"/>:posts.length?<><Reels posts={posts} user={user} onUpdate={update} onRemove={remove} notificationId={notificationId}/><div ref={sentinel} className="absolute bottom-6 left-1/2 z-50 -translate-x-1/2">{loadingMore&&<FiLoader className="animate-spin text-white/50"/></div></></>:<div className="grid h-full place-items-center bg-[#0B0F14] p-6"><EmptyState title="Nothing here yet" description="New posts and updates from the Emet community will appear in your Feed."/></div>}<QuoteModal post={quoteTarget} body={quoteBody} setBody={setQuoteBody} onClose={()=>{if(!quoteBusy)setQuoteTarget(null);}} onSubmit={submitQuote} busy={quoteBusy} error={quoteError}/></main>;
}
