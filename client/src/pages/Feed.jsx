...
function ActionMenu({post,onRemove,dark}) {
  ...
  return <div ref={menuRef} className="relative"><button type="button" onClick={()=>setOpen(v=>!v)} className={dark?'grid h-9 w-9 place-items-center rounded-full bg-black/35 text-white backdrop-blur-md hover:bg-black/55':'grid h-9 w-9 place-items-center rounded-full text-white/45 hover:bg-white/[.06]'} aria-label="More options"><FiMoreHorizontal/></button><AnimatePresence>{open&&<motion.div initial={{opacity:0,scale:.96,y:-4}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.96,y:-4}} className="absolute right-0 top-10 z-30 min-w-48 overflow-hidden rounded-xl border border-white/10 bg-[#171526]/95 p-1 shadow-2xl backdrop-blur-xl">
...
function QuoteModal({post,body,setBody,onClose,onSubmit,busy,error}) {
  return <AnimatePresence>{post&&<motion.div className="fixed inset-0 z-[100] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    <motion.div initial={{opacity:0,y:12,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:12,scale:.98}} className="w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-[#12111d] shadow-2xl">
