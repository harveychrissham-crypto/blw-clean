...
function ZoomableImage({src,onDoubleTap}) {
  ...
  const onPointerMove=e=>{
    ...
    pointers.current.set(e.pointerId,{x:e.clientX,y:e.clientY});
    const g=gesture.current;…3805 tokens truncated…reel===String(active));const next=Math.max(0,Math.min(cards.length-1,current+(direction==='next'?1:-1)));cards[next]?.scrollIntoView({block:'start',behavior:'smooth'});},[active]);
