...
import { FiAlertCircle, FiBookmark, FiChevronRight, FiFilm, FiHeart, FiLoader, FiMessageCircle, FiMoreHorizontal, FiSend, FiShare2, FiVolume2, FiVolumeX, FiWifiOff, FiX } from 'react-icons/fi';
...
function Comments({ post, user, onUpdate, compact = false }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      console.warn('[feed] comment failed:', error?.message || error);
      hapticError();
    } finally { setLoading(false); }
  };
          ...
          <div className="max-h-52 space-y-3 overflow-y-auto pr-1">
            ...
            {items.map((comment) => (
              <div key={comment.id} className="text-sm">
                <span className="font-semibold text-white/80">{comment.author_name || comment.authorName || 'Member'}</span>{' '}
                <span className="text-white/55">{comment.body}</span>
              </div>
            ))}
          </div>
          {user ? (
...
function ActionRow({ post, user, onUpdate }) {
  ...
  const share = () => {
    hapticTap();
    shareContent({ title: post.title, text: post.author ? `${post.title} — ${post.author}` : post.title, url: `${window.location.origin}/feed?notificationId=${encodeURIComponent(post.id)}` });
  };
  return (
    <div className="flex items-center gap-3 pt-3 text-white/70">
      ...
      <button type="button" onClick={() => document.getElementById(`comments-${post.id}`)?.click()} aria-label="Comments" className="inline-flex items-center gap-1.5 rounded-full p-2 transition hover:bg-white/5"><FiMessageCircle className="h-5 w-5" /><span className="text-xs">{post.commentCount.toLocaleString()}</span></button>
  ...
  return (
    <article id={`feed-${post.id}`} className={`overflow-hidden border-y border-white/[0.07] bg-[#0d0c18] sm:rounded-2xl sm:border ${target ? 'ring-2 ring-gold-500/60 ring-offset-2 ring-offset-ink-900' : ''}`}>
      ...
      <div className="px-4 pb-4 pt-3 sm:px-5">
        <span className="rounded-full bg-white/[0.06] px-2.5 py-1 text-[9px] font-bold uppercase tracking-widest text-white/45">{label}</span>
        <h2 className="mt-2 text-base font-bold text-white">{post.title}</h2>
        {post.body && <p className="mt-2 text-sm leading-relaxed text-white/60">{post.body}</p>}
        <ActionRow post={post} user={user} onUpdate={onUpdate} />
        <div className="mt-1 text-xs text-white/30">{post.saveCount.toLocaleString()} saves</div>
        <button id={`comments-${post.id}`} type="button" className="sr-only">Toggle comments</button>
        <Comments post={post} user={user} onUpdate={onUpdate} />
      </div>
    </article>
  );
}
...
function ReelCard({ post, user, onUpdate, notificationId, active }) {
  ...
  const share = () => {
    hapticTap();
    shareContent({ title: post.title, text: post.body || post.title, url: `${window.location.origin}/feed?notificationId=${encodeURIComponent(post.id)}` });
  };
  ...
  return (
    <article id={`feed-${post.id}`} className={`relative h-[calc(100dvh-9rem)] min-h-[540px] w-full snap-start overflow-hidden bg-black sm:h-[calc(100dvh-8rem)] sm:rounded-3xl sm:border sm:border-white/[0.08] ${target ? 'ring-2 ring-gold-500/60' : ''}`}>
      {post.videoId ? <iframe key={`${post.videoId}-${active}-${muted}`} className="absolute inset-0 h-full w-full scale-[1.01]" src={videoUrl} title={post.title || 'Feed Reel'} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen /> : post.image ? <img src={post.image} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 bg-black" />}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/45 via-transparent to-black/90" />
      <div className="absolute left-4 right-4 top-4 z-10 flex items-center justify-between safe-area-top">
        <span className="rounded-full bg-black/55 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-white/85 backdrop-blur">Reel</span>
        <button type="button" onClick={() => setMuted((value) => !value)} aria-label={muted ? 'Unmute reel' : 'Mute reel'} className="pointer-events-auto rounded-full bg-black/55 p-2.5 text-white backdrop-blur">{muted ? <FiVolumeX /> : <FiVolume2 />}</button>
      </div>
      <div className="absolute bottom-0 left-0 right-0 z-10 p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:p-7">
        <div className="flex items-end gap-5">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-3"><img src="/logo.png" alt="" className="h-11 w-11 rounded-full border border-white/20 object-cover" /><div><p className="text-sm font-bold text-white">{post.author}</p><p className="text-[11px] text-white/55">{post.time}</p></div></div>
            <h2 className="mt-3 text-base font-bold leading-snug text-white">{post.title}</h2>
            {post.body && post.body !== post.title && <p className="mt-1.5 line-clamp-3 text-sm leading-relaxed text-white/75">{post.body}</p>}
          </div>
          <div className="flex shrink-0 flex-col items-center gap-4 pb-1 text-white">
            <button type="button" onClick={async () => { if (!user) return hapticError(); try { const result = await toggleLike(post.id); onUpdate(post.id, result); hapticSuccess(); } catch { hapticError(); } }} className={`flex flex-col items-center gap-1 ${post.liked ? 'text-pink-400' : ''}`} aria-label="Like reel"><FiHeart className="h-7 w-7" fill={post.liked ? 'currentColor' : 'none'} /><span className="text-[11px]">{post.likeCount.toLocaleString()}</span></button>
            <button type="button" onClick={() => setCommentsOpen(true)} className="flex flex-col items-center gap-1" aria-label="Open comments"><FiMessageCircle className="h-7 w-7" /><span className="text-[11px]">{post.commentCount.toLocaleString()}</span></button>
            <button type="button" onClick={share} className="flex flex-col items-center gap-1" aria-label="Share reel"><FiShare2 className="h-7 w-7" /></button>
            <button type="button" onClick={async () => { if (!user) return hapticError(); try { const result = await toggleSave(post.id); onUpdate(post.id, result); hapticSuccess(); } catch { hapticError(); } }} className="flex flex-col items-center gap-1" aria-label="Save reel"><FiBookmark className="h-7 w-7" fill={post.saved ? 'currentColor' : 'none'} /></button>
          </div>
        </div>
      </div>
      {commentsOpen && <div className="absolute inset-x-0 bottom-0 z-20 max-h-[70%] overflow-y-auto rounded-t-3xl bg-[#0d0c18]/95 p-5 backdrop-blur-xl"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold text-white">Comments</h3><button type="button" onClick={() => setCommentsOpen(false)} className="rounded-full p-2 text-white/60" aria-label="Close comments"><FiX /></button></div><Comments post={post} user={user} onUpdate={onUpdate} /></div>}
    </article>
  );
}
...
function ReelFeed({ posts, user, onUpdate, notificationId }) {
  ...
  return <div ref={containerRef} className="h-[calc(100dvh-9rem)] snap-y snap-mandatory overflow-y-auto overscroll-contain scroll-smooth sm:h-[calc(100dvh-8rem)]" style={{ scrollbarWidth: 'none' }}>{posts.map((post) => <div key={post.id} data-reel-id={String(post.id)} className="h-[calc(100dvh-9rem)] min-h-[540px] snap-start sm:h-[calc(100dvh-8rem)]"><ReelCard post={post} user={user} onUpdate={onUpdate} notificationId={notificationId} active={String(activeId) === String(post.id)} /></div>)}</div>;
}
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-white" {...bind}>
      ...
      <div className="mx-auto max-w-3xl px-0 pb-24 sm:px-4 sm:pt-6">
        <div className="px-4 pt-4 sm:px-0"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/35">BLW Kenya Zone</p><h1 className="mt-1 text-2xl font-bold">Feed</h1></div><FiFilm className="h-6 w-6 text-white/40" /></div></div>
        <div className="mt-4"><StoriesRow stories={stories} /></div>
        <div className="sticky top-0 z-20 mt-2 border-b border-white/[0.06] bg-ink-950/95 px-4 py-2 backdrop-blur-xl sm:rounded-xl">
          <div className="flex gap-1 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>{tabs.map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`shrink-0 rounded-full px-4 py-2 text-xs font-semibold transition ${activeTab === tab ? 'bg-white text-ink-950' : 'text-white/45 hover:bg-white/5 hover:text-white'}`}>{tab}</button>)}</div>
        </div>
