...
import { FiEdit2, FiMapPin, FiPlus, FiSave, FiTrash2, FiX } from 'react-icons/fi';
...
function CenterOnLocation({ latitude, longitude }) {
  useMapEvents({
    click() {},
  });
  return null;
...
export default function AdminDashboard() {
  ...
  const mapCenter = validCoords ? [latitude, longitude] : DEFAULT_CENTER;

  const countries = useMemo(() => {
    return [...new Set(locations.map((location) => location.country).filter(Boolean))];
  }, [locations]);
  ...
  const startEdit = (location) => {
    ...
    setForm({
      fellowshipName: location.fellowshipName || '', country: location.country || 'Kenya',
      city: location.city || '', town: location.town || '', area: location.area || '',
      university: location.university || '', address: location.address || '',
      description: location.description || '', serviceTime: location.serviceTime || '',
      latitude: location.latitude ?? '', longitude: location.longitude ?? '', isActive: location.isActive !== false,
    });
    setNotice('');
    setError('');
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {(error || notice) && (
        <div className={`mb-6 rounded-2xl border px-4 py-3 text-sm ${error ? 'border-red-500/20 bg-red-500/10 text-red-300' : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300'}`}>
          {error || notice}
        </div>
      )}
      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <form onSubmit={saveLocation} className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-soft sm:p-7">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-[#F2A31C]">{editingId ? 'Edit location' : 'Add location'}</p>
              <h2 className="mt-1 text-2xl font-bold text-white">{editingId ? 'Update fellowship' : 'New fellowship'}</h2>
            </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {[
              ...
              <label key={name} className="block">
                ...
                <input value={form[name]} onChange={(event) => setField(name, event.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none focus:border-[#A53DFF]" />
              </label>
            ))}
          </div>

          <label className="mt-4 block">
            <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/45">Description</span>
            <textarea value={form.description} onChange={(event) => setField('description', event.target.value)} className="min-h-[100px] w-full rounded-xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none focus:border-[#A53DFF]" />
          </label>
          ...
          <label className="mt-4 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/75">
            <input type="checkbox" checked={form.isActive} onChange={(event) => setField('isActive', event.target.checked)} />
            Show this fellowship in public search
          </label>

          <div className="mt-6 flex flex-wrap gap-3">
            <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#EC2FA8] via-[#8A2BE2] to-[#3D5AFE] px-5 py-3 text-sm font-bold text-white disabled:opacity-50"><FiSave /> {saving ? 'Saving...' : editingId ? 'Save Changes' : 'Add Fellowship'}</button>
            <button type="button" onClick={() => setForm({ ...emptyForm })} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white/80 hover:bg-white/10"><FiMapPin /> Reset Pin</button>
          </div>
        </form>

        <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-4 shadow-soft sm:p-5">
          <div className="flex items-center justify-between gap-4 px-2 pb-4">
            <div><p className="text-xs font-bold uppercase tracking-widest text-[#F2A31C]">Map picker</p><h2 className="mt-1 text-xl font-bold text-white">Click the exact fellowship location</h2></div>
            <span className="text-xs text-white/35">{countries.length} countries</span>
          </div>
          <div className="overflow-hidden rounded-2xl border border-white/10">
            <MapContainer center={mapCenter} zoom={validCoords ? 16 : 6} scrollWheelZoom className="h-[480px] w-full">
              <TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
              <MapClickPicker onPick={pickLocation} />
              {validCoords && <CircleMarker center={mapCenter} radius={10} pathOptions={{ color: '#EC2FA8', fillColor: '#8A2BE2', fillOpacity: 0.85 }} />}
            </MapContainer>
          </div>
          <p className="mt-3 px-2 text-xs text-white/35">Click anywhere on the map to set the pin. You can also fine-tune latitude and longitude manually.</p>
        </div>
      </div>
      ...
      <div className="mt-10 rounded-3xl border border-white/10 bg-slate-900/70 p-5 shadow-soft sm:p-6">
        ...
        <div className="mt-5 space-y-3">
          {loading ? <div className="py-8 text-center text-sm text-white/30">Loading fellowship locations...</div> : locations.length === 0 ? <div className="py-8 text-center text-sm text-white/30">No fellowship locations have been added yet.</div> : locations.map((location) => (
