...
export default function LeaderVideoAdmin() {
  ...
  return <section className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.25em] text-gold-500">Leader Admin</p><h1 className="mt-2 text-4xl font-bold">Video & Livestream</h1><p className="mt-3 text-white/60">Start the main service broadcast and manage active fellowship rooms.</p></div><button onClick={load} disabled={busy === 'load'} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-white/75"><FiRefreshCw className={busy === 'load' ? 'animate-spin' : ''}/> Refresh</button></div>
