...
export default function LeaderVideoAdmin() {
  ...
  return <section className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.25em] text-gold-500">Leader Admin</p><h1 className="mt-2 text-4xl font-bold">Video & Livestream</h1><p className="mt-3 text-white/60">Start the main service broadcast and manage active fellowship rooms.</p></div><Button variant="custom" size="none" onClick={load} disabled={busy === 'load'} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-white/75"><FiRefreshCw className={busy === 'load' ? 'animate-spin' : ''}/> Refresh</Button></div>
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-2">
      <Card variant="custom" className="rounded-3xl border border-white/10 bg-white/[0.04] p-6"><div className="flex items-center gap-3"><FiRadio className="h-6 w-6 text-gold-500"/><h2 className="text-xl font-semibold">Main livestream</h2></div><p className="mt-2 text-sm text-white/55">Broadcast the main service room as HLS for the public Live page.</p>
        ...
        {broadcast ? <><Card variant="custom" className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-4"><p className="text-sm font-semibold text-emerald-200">Broadcast active</p><p className="mt-1 text-sm text-white/70">{broadcast.title || title}</p><p className="mt-1 break-all text-xs text-white/50">{broadcast.playback_url}</p></Card><Button variant="custom" size="none" onClick={stop} disabled={busy === 'stop'} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-red-400/20 py-3 font-semibold text-red-200"><FiStopCircle/>{busy === 'stop' ? 'Stopping…' : 'Stop livestream'}</Button></> : <Button variant="custom" size="none" onClick={start} disabled={busy === 'start'} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-pink-600 via-purple-500 to-indigo-500 py-3 font-semibold">{busy === 'start' ? 'Starting…' : 'Start livestream'}</Button>}
      </Card>
