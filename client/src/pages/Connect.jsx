...
export default function Connect() {
  ...
  const fetchApproximateLocation = async () => {
    try {
      ...
      if (!response.ok || !Number.isFinite(latitude) || !Number.isFinite(longitude)) throw new Error('Approximate location is unavailable.');
      setUserLocation(null);
      setCampusSearch('Where I am');
      applyNearbyLocation(latitude, longitude, Array.isArray(body.fellowships) ? body.fellowships : [], `Near ${body.location.city || 'your area'}`, false);
      return true;
    } catch {
      return false;
    }
  ...
  const searchPlace = async () => {
    ...
    try {
      ...
      if (!result || !Number.isFinite(Number(result.lat)) || !Number.isFinite(Number(result.lon))) throw new Error(`No map location was found for “${query}”.`);
      const shortLabel = result.address?.town || result.address?.city || result.address?.municipality || result.address?.suburb || query;
      applyNearbyLocation(Number(result.lat), Number(result.lon), Array.isArray(fellowshipBody.fellowships) ? fellowshipBody.fellowships : [], shortLabel, false);
    } catch (error) {
      setNearbyFellowships([]);
  ...
  const useMyLocation = () => {
    ...
    if (!navigator.geolocation) {
      fetchApproximateLocation().then((ok) => { setLocating(false); if (!ok) setMapMessage('Location is unavailable on this device.'); });
      return;
    }
        ...
        <Card as={motion.div} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} variant="raised" className="p-8 shadow-soft">
          <div>
            ...
            <Card variant="subtle" className="mt-7 p-4 sm:p-5">
              <div className="mb-4 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#8A2BE2]/15 text-[#D8B2FF]"><FiSearch className="h-5 w-5" /></div><div><p className="text-sm font-bold text-white">Find your nearest fellowship</p><p className="text-xs text-white/60">Search a place or use your current location.</p></div></div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row">
                <div className="relative min-w-0 flex-1">
                  ...
                  {suggestions.length > 0 && <div className="absolute left-0 right-0 top-[calc(100%+10px)] z-30 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/98 shadow-2xl backdrop-blur-xl">{suggestions.map((location) => <button key={location.id} type="button" onClick={() => chooseSuggestion(location)} className="flex w-full items-start gap-3 border-b border-white/5 px-4 py-3.5 text-left transition last:border-b-0 hover:bg-white/[0.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[#8A2BE2]/15 text-[#D8B2FF]"><FiMapPin className="h-4 w-4" /></span><span className="min-w-0"><span className="block truncate text-sm font-semibold text-white">{location.fellowshipName}</span><span className="mt-1 block truncate text-xs text-white/60">{[location.town || location.city, location.area, location.university, location.country].filter(Boolean).join(' • ')}</span></span></button>)}</div>}
                </div>
              ...
              {(selectedPlace || nearbyFellowships.length > 0 || mapMessage) && <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-slate-950/50">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-3"><div><p className="text-sm font-semibold text-white">{selectedPlace === 'Where I am' ? 'Your current area' : selectedPlace || 'Selected place'}</p><p className="mt-0.5 text-xs text-white/35">Fellowships within {NEARBY_RADIUS_KM} km</p></div><span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-white/55">{nearbyFellowships.length} nearby</span></div>
