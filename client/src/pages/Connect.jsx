            ...
            <Card variant="subtle" className="mt-7 p-4 sm:p-5">
              ...
              <div className="flex flex-col gap-3 lg:flex-row">
                <div className="relative min-w-0 flex-1">
                  ...
                  {suggestions.length > 0 && <div className="absolute left-0 right-0 top-[calc(100%+10px)] z-30 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/98 shadow-2xl backdrop-blur-xl">{suggestions.map((location) => <button key={location.id} type="button" onClick={() => chooseSuggestion(location)} className="flex w-full items-start gap-3 border-b border-white/5 px-4 py-3.5 text-left transition last:border-b-0 hover:bg-white/[0.05]"><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[#8A2BE2]/15 text-[#D8B2FF]"><FiMapPin className="h-4 w-4" /></span><span className="min-w-0"><span className="block truncate text-sm font-semibold text-white">{location.fellowshipName}</span><span className="mt-1 block truncate text-xs text-white/40">{[location.town || location.city, location.area, location.university, location.country].filter(Boolean).join(' • ')}</span></span></button>)}</div>}
                </div>
