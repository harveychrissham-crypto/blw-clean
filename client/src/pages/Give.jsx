    ...
    <section className="mx-auto max-w-6xl px-5 py-16">
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="rounded-2xl p-6" style={card}>
          <div className="mb-4 flex items-center gap-2 text-xs font-bold uppercase tracking-widest" style={{ color: '#F2A31C' }}><FiCreditCard /> Payment Options</div>
          <div className="space-y-3">
            {options.map((item) => <button key={item.name} type="button" onClick={() => item.name === 'M-Pesa' && openMpesa()} className="block w-full rounded-xl p-4 text-left transition hover:bg-white/10" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}><div className="text-sm font-semibold text-white">{item.name}</div><div className="mt-0.5 text-xs text-white/40">{item.desc}</div></button>)}
          </div>
        </div>
      </div>
        ...
        {mpesaOpen && (
          <motion.div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMpesaOpen(false)}>
            <motion.div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl p-6 shadow-2xl sm:p-8" style={{ background: '#101018', border: '1px solid rgba(255,255,255,0.1)' }} initial={{ opacity: 0, scale: 0.96, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 20 }} onClick={(event) => event.stopPropagation()}>
              {!selectedGiving ? (
                <>
                  <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest" style={{ color: '#F2A31C' }}>M-Pesa Giving</p><h3 className="mt-2 text-xl font-extrabold text-white sm:text-2xl">Choose what you would like to give towards</h3></div><button type="button" onClick={() => setMpesaOpen(false)} className="rounded-full p-2 text-white/60 hover:bg-white/10 hover:text-white"><FiX size={20} /></button></div>
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{mpesaAccounts.map((item) => <button key={item.name} type="button" onClick={() => selectGiving(item)} className="rounded-xl p-4 text-left transition hover:bg-white/10" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}><div className="text-sm font-semibold text-white">{item.name}</div><div className="mt-2 flex items-center justify-between"><span className="font-mono text-lg font-bold" style={{ color: '#F2A31C' }}>{item.account}</span><span className="text-xs text-white/40">Select</span></div></button>)}</div>
                </>
              ) : (
                <>
                  <div className="flex items-start justify-between gap-4"><div><button type="button" onClick={() => setSelectedGiving(null)} className="mb-3 inline-flex items-center gap-2 text-xs font-semibold text-white/50 hover:text-white"><FiArrowLeft /> Back</button><p className="text-xs font-bold uppercase tracking-widest" style={{ color: '#F2A31C' }}>M-Pesa Giving</p><h3 className="mt-2 text-2xl font-extrabold text-white">{selectedGiving.name}</h3><p className="mt-1 text-sm text-white/45">Account {selectedGiving.account} · Paybill {MPESA_PAYBILL}</p></div><button type="button" onClick={() => setMpesaOpen(false)} className="rounded-full p-2 text-white/60 hover:bg-white/10 hover:text-white"><FiX size={20} /></button></div>
