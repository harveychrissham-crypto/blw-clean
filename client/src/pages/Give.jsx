      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <Card as={motion.div} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} variant="raised" className="p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button type="button" onClick={openMpesa} className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-bold text-white transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40" style={{ background: 'linear-gradient(135deg,#EC2FA8,#8A2BE2)' }}><FiDollarSign /> Give Now</button>
            <button type="button" onClick={openMpesa} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiCreditCard /> M-Pesa Giving</button>
          </div>
        </Card>
          ...
          <motion.div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMpesaOpen(false)}>
            <motion.div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl p-6 shadow-2xl sm:p-8" style={{ background: '#101018', border: '1px solid rgba(255,255,255,0.1)' }} initial={{ opacity: 0, scale: 0.96, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 20 }} onClick={(event) => event.stopPropagation()}>
              {!selectedGiving ? (
                ...
                <>
                  <div className="flex items-start justify-between gap-4"><div><button type="button" onClick={() => setSelectedGiving(null)} className="mb-3 inline-flex items-center gap-2 text-xs font-semibold text-white/50 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiArrowLeft /> Back</button><Eyebrow>M-Pesa Giving</Eyebrow><h3 className="mt-2 text-2xl font-extrabold text-white">{selectedGiving.name}</h3><div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-white/45"><span>Account {selectedGiving.account} · Paybill {MPESA_PAYBILL}</span><button type="button" onClick={() => copyText(selectedGiving.account, 'Account number')} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1 text-xs font-semibold text-white/70 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40">{copied === selectedGiving.account ? <FiCheck /> : <FiCopy />}{copied === selectedGiving.account ? 'Copied' : 'Copy account'}</button></div></div><button type="button" onClick={() => setMpesaOpen(false)} className="rounded-full p-2 text-white/60 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiX size={20} /></button></div>
                  <form onSubmit={sendPrompt} className="mt-7 space-y-5">
                    ...
                    <button type="submit" className="w-full rounded-xl px-5 py-3.5 text-sm font-bold text-white transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40" style={{ background: 'linear-gradient(135deg,#EC2FA8,#8A2BE2)' }}>Send M-Pesa Prompt</button>
                  </form>
                </>
              )}
            </motion.div>
