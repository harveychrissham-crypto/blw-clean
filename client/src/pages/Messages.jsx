...
export default function Messages() {
  ...
  if (!user) {
    return <section className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-2xl place-items-center px-4 pb-24 text-center">
      <div><div className="mx-auto grid h-20 w-20 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-white/65"><FiMessageCircle className="h-8 w-8" /></div><h1 className="mt-6 text-xl font-bold text-white">Sign in to use Messages</h1><p className="mt-2 text-sm text-white/40">Connect privately with people in the Emet community.</p><Link to="/auth" className="mt-5 inline-flex rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-[#0B0F14]">Sign in</Link></div>
    </section>;
  }

  return <section className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl flex-col px-3 pb-24 pt-3 sm:px-6 sm:pt-6">
    <div className="mb-3 flex items-center justify-between px-1 sm:px-0"><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-white/30">Emet · Messages</p><h1 className="text-xl font-bold text-white">Messages</h1></div><Link to="/feed" className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-semibold text-white/70 hover:bg-white/[0.08] hover:text-white">Back to Feed</Link></div>
