...
export default function Messages() {
  ...
  if (!user) {
    return <section className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-2xl place-items-center px-4 pb-24 text-center">
      <div><div className="mx-auto grid h-20 w-20 place-items-center rounded-full border border-white/12 bg-[#11161D] text-white/65"><FiMessageCircle className="h-8 w-8" /></div><h1 className="mt-6 text-xl font-bold text-white">Sign in to use Messages</h1><p className="mt-2 text-sm text-white/40">Connect privately with people in the Emet community.</p><Link to="/auth" className="mt-5 inline-flex rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-[#0B0F14] hover:bg-white/90">Sign in</Link></div>
    </section>;
  }
    ...
    <div className="flex min-h-0 flex-1 overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.025] shadow-2xl shadow-black/20">
      ...
      <main className={`min-w-0 flex-1 flex-col bg-[#0B0F14]/35 ${selected ? 'flex' : 'hidden sm:flex'}`}>
        {selected ? <>
          ...
          <div ref={messageListRef} className="flex-1 overflow-y-auto px-4 py-5 sm:px-6">{messages.length ? <div className="mx-auto flex max-w-2xl flex-col gap-2">{messages.map((message) => { const mine = message.senderEmail?.toLowerCase() === user.email?.toLowerCase(); return <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${mine ? 'rounded-br-md bg-white text-[#0B0F14]' : 'rounded-bl-md border border-white/[0.08] bg-white/[0.055] text-white/90'}`}><p className="whitespace-pre-wrap break-words">{message.body}</p><p className={`mt-1 text-[9px] ${mine ? 'text-[#0B0F14]/45' : 'text-white/25'}`}>{formatTime(message.createdAt)}</p></div></div>; })}</div> : <div className="grid h-full place-items-center text-center"><div><Avatar person={selected.other} size="h-16 w-16 mx-auto" /><h3 className="mt-4 font-bold text-white">{selected.other?.name}</h3><p className="mt-1 text-xs text-white/35">Send the first message.</p></div></div>}</div>
          <form onSubmit={sendMessage} className="border-t border-white/[0.07] p-3 sm:p-4"><div className="mx-auto flex max-w-2xl items-end gap-2 rounded-2xl border border-white/10 bg-white/[0.045] p-2"><textarea value={composer} onChange={(event) => setComposer(event.target.value.slice(0, 2000))} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} rows={1} placeholder="Message…" className="max-h-28 min-h-10 flex-1 resize-none bg-transparent px-2 py-2 text-sm text-white outline-none placeholder:text-white/30" /><button disabled={!composer.trim() || sending} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-[#0B0F14] transition hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-30" aria-label="Send message"><FiSend className="h-4 w-4" /></button></div></form>
