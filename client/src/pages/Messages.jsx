...
export default function Messages() {
  ...
  if (!user) {
    return <section className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-2xl place-items-center px-4 pb-24 text-center">
      <div><div className="mx-auto grid h-20 w-20 place-items-center rounded-full border border-white/12 bg-[#11161D] text-white/65"><FiMessageCircle className="h-8 w-8" /></div><h1 className="mt-6 text-xl font-bold text-white">Sign in to use Messages</h1><p className="mt-2 text-sm text-white/40">Connect privately with people in the Emet community.</p><Link to="/auth" className="mt-5 inline-flex rounded-xl bg-[#E7E9EA] px-5 py-2.5 text-sm font-semibold text-[#0B0F14] hover:bg-[#D9DDE1]">Sign in</Link></div>
    </section>;
  }
    ...
    <div className="flex min-h-0 flex-1 overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.025] shadow-2xl shadow-black/20">
      <aside className={`w-full shrink-0 border-white/[0.07] sm:w-[340px] sm:border-r ${selected ? 'hidden sm:block' : 'block'}`}>
        <div className="border-b border-white/[0.07] px-4 pb-3 pt-4">
          <div className="flex items-center gap-3"><Link to="/feed" aria-label="Back to feed" className="grid h-9 w-9 place-items-center rounded-xl text-white/60 hover:bg-white/[0.06] hover:text-white"><FiArrowLeft /></Link><div><h2 className="text-sm font-bold text-white">Inbox</h2><p className="text-[10px] text-white/30">Private conversations</p></div></div>
          <div className="mt-3 flex items-center gap-2 rounded-2xl border border-white/12 bg-[#11161D] px-3 py-2.5"><FiSearch className="h-4 w-4 text-white/35" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search members or conversations" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/30" />{query && <button onClick={() => setQuery('')} className="text-white/35 hover:text-white" aria-label="Clear search"><FiX /></button>}</div>
        </div>
        ...
        <div className="overflow-y-auto">
          {loading ? <div className="space-y-2 p-3">{[1,2,3,4].map((item) => <div key={item} className="h-16 animate-pulse rounded-2xl bg-white/[0.035]" />)}</div> : filtered.length ? filtered.map((conversation) => <button key={conversation.id} onClick={() => openConversation(conversation)} className={`flex w-full items-center gap-3 px-4 py-3.5 text-left transition hover:bg-white/[0.045] ${conversation.id === conversationId ? 'bg-white/[0.06]' : ''}`}><Avatar person={conversation.other} /><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><span className={`truncate text-sm ${conversation.unreadCount ? 'font-bold text-white' : 'font-semibold text-white/90'}`}>{conversation.other?.name}</span><span className="shrink-0 text-[10px] text-white/30">{formatTime(conversation.lastMessageAt)}</span></span><span className="mt-1 flex items-center gap-2"><span className={`truncate text-xs ${conversation.unreadCount ? 'font-medium text-white/65' : 'text-white/35'}`}>{conversation.lastMessage || 'Start a conversation'}</span>{conversation.unreadCount > 0 && <span className="grid h-5 min-w-5 shrink-0 place-items-center rounded-full bg-white px-1 text-[10px] font-bold text-[#0B0F14]">{conversation.unreadCount}</span>}</span></span></button>) : <div className="px-5 py-16 text-center"><FiMessageCircle className="mx-auto h-7 w-7 text-white/25" /><p className="mt-4 text-sm font-semibold text-white/70">{query ? 'No matches' : 'No conversations yet'}</p><p className="mt-1 text-xs text-white/35">{query ? 'Try another name.' : 'Search for a member to start chatting.'}</p></div>}
        </div>
      </aside>

      <main className={`min-w-0 flex-1 flex-col bg-[#0B0F14]/35 ${selected ? 'flex' : 'hidden sm:flex'}`}>
        {selected ? <>
          <header className="flex items-center gap-3 border-b border-white/[0.07] px-4 py-3.5"><button onClick={() => { setSelected(null); setParams({ messages: '1' }); }} className="grid h-9 w-9 place-items-center rounded-xl text-white/60 hover:bg-white/[0.06] sm:hidden" aria-label="Back to conversations"><FiArrowLeft /></button><Avatar person={selected.other} size="h-10 w-10" /><div className="min-w-0"><h2 className="truncate text-sm font-bold text-white">{selected.other?.name}</h2><p className="truncate text-xs text-white/35">{selected.other?.title || selected.other?.chapter || 'Emet member'}</p></div></header>
          <div ref={messageListRef} className="flex-1 overflow-y-auto px-4 py-5 sm:px-6">{messages.length ? <div className="mx-auto flex max-w-2xl flex-col gap-2">{messages.map((message) => { const mine = message.senderEmail?.toLowerCase() === user.email?.toLowerCase(); return <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${mine ? 'rounded-br-md bg-[#E7E9EA] text-[#0B0F14]' : 'rounded-bl-md border border-white/[0.08] bg-white/[0.055] text-white/90'}`}><p className="whitespace-pre-wrap break-words">{message.body}</p><p className={`mt-1 text-[9px] ${mine ? 'text-[#0B0F14]/45' : 'text-white/25'}`}>{formatTime(message.createdAt)}</p></div></div>; })}</div> : <div className="grid h-full place-items-center text-center"><div><Avatar person={selected.other} size="h-16 w-16 mx-auto" /><h3 className="mt-4 font-bold text-white">{selected.other?.name}</h3><p className="mt-1 text-xs text-white/35">Send the first message.</p></div></div>}</div>
          <form onSubmit={sendMessage} className="border-t border-white/[0.07] p-3 sm:p-4"><div className="mx-auto flex max-w-2xl items-end gap-2 rounded-2xl border border-white/10 bg-white/[0.045] p-2"><textarea value={composer} onChange={(event) => setComposer(event.target.value.slice(0, 2000))} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} rows={1} placeholder="Message…" className="max-h-28 min-h-10 flex-1 resize-none bg-transparent px-2 py-2 text-sm text-white outline-none placeholder:text-white/30" /><button disabled={!composer.trim() || sending} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#E7E9EA] text-[#0B0F14] transition hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-30" aria-label="Send message"><FiSend className="h-4 w-4" /></button></div></form>
