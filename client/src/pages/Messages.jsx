...
import { FiArrowLeft, FiMessageCircle, FiSearch, FiSend, FiUserPlus, FiX } from 'react-icons/fi';
  ...
  return <section className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl flex-col px-3 pb-24 pt-3 sm:px-6 sm:pt-6">
    <div className="flex min-h-0 flex-1 overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.025] shadow-2xl shadow-black/20">
      <aside className={`w-full shrink-0 border-white/[0.07] sm:w-[340px] sm:border-r ${selected ? 'hidden sm:block' : 'block'}`}>
        <div className="border-b border-white/[0.07] px-4 pb-3 pt-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3"><Link to="/feed" aria-label="Back to feed" className="grid h-9 w-9 place-items-center rounded-full text-white/60 hover:bg-white/[0.06] hover:text-white"><FiArrowLeft /></Link><h1 className="text-lg font-bold text-white">Messages</h1></div>
            <Link to="/connect" className="grid h-9 w-9 place-items-center rounded-full text-white/60 hover:bg-white/[0.06] hover:text-white" aria-label="Find people"><FiUserPlus /></Link>
          </div>
          <div className="mt-3 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2.5"><FiSearch className="h-4 w-4 text-white/35" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people or conversations" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/30" />{query && <button onClick={() => setQuery('')} className="text-white/35 hover:text-white" aria-label="Clear search"><FiX /></button>}</div>
        </div>
      ...
      <main className={`min-w-0 flex-1 flex-col bg-[#08080d]/35 ${selected ? 'flex' : 'hidden sm:flex'}`}>
        {selected ? <>
          <header className="flex items-center gap-3 border-b border-white/[0.07] px-4 py-3.5"><button onClick={() => { setSelected(null); setParams({}); }} className="grid h-9 w-9 place-items-center rounded-full text-white/60 hover:bg-white/[0.06] sm:hidden" aria-label="Back to conversations"><FiArrowLeft /></button><Avatar person={selected.other} size="h-10 w-10" /><div className="min-w-0"><h2 className="truncate text-sm font-bold text-white">{selected.other?.name}</h2><p className="truncate text-xs text-white/35">{selected.other?.title || selected.other?.chapter || 'BLW Kenya Zone member'}</p></div></header>
