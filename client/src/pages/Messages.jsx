...
export default function Messages() {
  ...
  if (!user) {
    return <section className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-2xl place-items-center px-4 pb-24 text-center">
      <div><div className="mx-auto grid h-20 w-20 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-white/65"><FiMessageCircle className="h-8 w-8" /></div><h1 className="mt-6 text-xl font-bold text-white">Sign in to use Messages</h1><p className="mt-2 text-sm text-white/40">Connect privately with people in the Emet community.</p><Link to="/auth" className="mt-5 inline-flex rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-[#0B0F14] hover:bg-white/90">Sign in</Link></div>
    </section>;
  }
