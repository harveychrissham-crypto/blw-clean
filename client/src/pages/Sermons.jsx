  ...
  return (
    <div className="flex gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <div className="min-w-0">
        ...
        {sermon.speaker && (
          <p className="mt-1 text-sm font-semibold" style={{ color: '#F2A31C' }}>
            {sermon.speaker}
          </p>
        )}
...
export default function Sermons() {
  ...
  /**
   * The featured sermon is public. Every other sermon requires an account.
   */
  const activeSermon =
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-6 sm:py-14">
      <div className="mb-8">
        <p className="mb-3 text-xs font-bold uppercase tracking-widest" style={{ color: '#F2A31C' }}>
          Sermons
        </p>
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/50">
          Watch sermons and teachings from Believers' LoveWorld Campus Ministry.
        </p>
      </div>
      ...
      {!loading && activeSermon && (
        ...
        >
          ...
          <div className="pt-1">
            ...
            <h2 className="text-2xl font-extrabold leading-tight text-white sm:text-3xl" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              {activeSermon.title}
            </h2>

            {activeSermon.speaker && (
              <p className="mt-2 text-sm font-semibold" style={{ color: '#F2A31C' }}>
                {activeSermon.speaker}
              </p>
            )}
        ...
        <div className="mt-14">
          ...
          <div className="space-y-10">
            {olderSermons.map((sermon, index) => {
              if (authLoading) {
                return (
                  <div key={sermon.id} className="flex aspect-video w-full items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] text-sm text-white/30">
