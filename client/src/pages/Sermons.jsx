...
import { FiAlertCircle, FiFilm, FiWifiOff, FiShare2, FiHeart, FiMessageCircle, FiSend, FiMoreHorizontal, FiBookmark, FiLoader } from 'react-icons/fi';
...
function SermonPlayer({ sermon }) {
  ...
  if (!sermon?.youtube_id) return <Card variant="subtle" className="flex aspect-square w-full items-center justify-center text-sm text-white/60 sm:aspect-video"><div className="text-center"><FiFilm className="mx-auto mb-2 h-8 w-8" /><p>Video unavailable</p></div></Card>;
...
function FeedPost({ post, index, user, notificationId, onUpdate }) {
  ...
  return <article id={`sermon-${post.id}`} className={`overflow-hidden border-y border-white/[0.07] bg-[#0d0c18] sm:rounded-2xl sm:border ${isTarget ? 'ring-2 ring-gold-500/60 ring-offset-2 ring-offset-ink-900' : ''}`}>
...
export default function Sermons() {
  ...
  const updatePost = (id, patch) => setPosts((current) => current.map((post) => post.id === id ? { ...post, ...(typeof patch.liked === 'boolean' ? { liked: patch.liked } : {}), ...(typeof patch.saved === 'boolean' ? { saved: patch.saved } : {}), ...(typeof patch.likeCount === 'number' ? { like_count: patch.likeCount } : {}), ...(typeof patch.saveCount === 'number' ? { save_count: patch.saveCount } : {}), ...(typeof patch.commentCount === 'number' ? { comment_count: patch.commentCount } : {}) } : post));
