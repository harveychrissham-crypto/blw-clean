  ...
  return <main className="min-h-screen bg-[#0B0F14] pb-28 text-white">
    <div className="mx-auto max-w-4xl">
        ...
        <div className="px-5 pb-5 pt-16 sm:px-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="shrink-0">
              {isPublic ? <button onClick={doFollow} disabled={followBusy} className={following ? 'rounded-xl border border-white/12 bg-[#11161D] px-5 py-2.5 text-sm font-semibold hover:border-[#1D9BF0]/40 hover:bg-white/[.08]' : 'rounded-xl bg-[#E7E9EA] text-[#0B0F14] px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#D9DDE1]'}>{followBusy ? '…' : following ? 'Following' : 'Follow'}</button> : <button onClick={() => setEditing(true)} className="rounded-xl border border-white/15 px-5 py-2.5 text-sm font-semibold hover:border-[#1D9BF0]/40 hover:bg-white/[.05]"><FiEdit3 className="mr-2 inline" />Edit profile</button>}
            </div>
          </div>
      ...
      <nav className="sticky top-[57px] z-20 border-b border-white/10 bg-[#0B0F14]/95">
        <div className="grid grid-cols-4">{TABS.map(t => <button key={t} onClick={() => setTab(t)} className="relative py-4 text-sm font-bold text-white/45 hover:text-white">{t}{tab === t && <span className="absolute inset-x-1/3 bottom-0 h-1 rounded-full bg-[#E7E9EA] text-[#0B0F14]" />}</button>)}</div>
      </nav>

      {error && <div className="mx-5 mt-4 rounded-xl border border-[#1D9BF0]/30 bg-[#E7E9EA] text-[#0B0F14]/10 p-3 text-sm text-white">{error}</div>}
