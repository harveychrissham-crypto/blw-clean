  ...
  return <main className="min-h-screen bg-[#0B0F14] pb-28 text-white">
    <div className="mx-auto max-w-4xl">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-white/45">
                ...
                {profile?.website && <a href={profile.website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#3B82F6]"><FiLink />{profile.website.replace(/^https?:\/\//, '')}</a>}
      ...
      <nav className="sticky top-[57px] z-20 border-b border-white/10 bg-[#0B0F14]/95">
        <div className="grid grid-cols-4">{TABS.map(t => <button key={t} onClick={() => setTab(t)} className="relative py-4 text-sm font-bold text-white/45 hover:text-white">{t}{tab === t && <span className="absolute inset-x-1/3 bottom-0 h-1 rounded-full bg-[#3B82F6]" />}</button>)}</div>
      </nav>

      {error && <div className="mx-5 mt-4 rounded-xl border border-[#3B82F6]/30 bg-[#3B82F6]/10 p-3 text-sm text-white">{error}</div>}
