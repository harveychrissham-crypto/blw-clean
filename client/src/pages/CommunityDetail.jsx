...
export default function CommunityDetail() {
  ...
  if (!data?.community) return <main className="grid min-h-screen place-items-center bg-[#0B0F14] px-6 text-center text-white"><div><p className="font-bold">{error || 'Community not found.'}</p><button onClick={() => navigate('/communities')} className="mt-4 rounded-full bg-white px-5 py-2.5 text-sm font-bold text-[#0B0F14]">Back to communities</button></div></main>;
  ...
  return <main className="min-h-screen bg-[#0B0F14] pb-28 text-white">
    <div className="mx-auto max-w-3xl px-4 py-5 sm:px-6 sm:py-8">
      <header className="flex items-center gap-3"><button onClick={() => navigate('/communities')} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[.03] text-white/65"><FiArrowLeft /></button><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[.2em] text-[#3B82F6]">Community</p><h1 className="truncate text-2xl font-extrabold">{community.name}</h1></div></header>
      <section className="mt-6 rounded-3xl border border-white/[.07] bg-white/[.025] p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-sm leading-6 text-white/55">{community.description}</p><div className="mt-4 flex items-center gap-2 text-xs text-white/35"><FiUsers /> {Number(community.member_count || 0).toLocaleString()} members · {community.category}</div></div><button onClick={toggleJoin} className={`shrink-0 rounded-full px-4 py-2 text-xs font-bold ${community.joined ? 'border border-white/10 bg-white/[.05] text-white/60' : 'bg-[#3B82F6] text-white'}`}>{community.joined ? 'Joined' : 'Join'}</button></div></section>
      {error && <div className="mt-4 rounded-2xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200">{error}</div>}
      {community.joined && <form onSubmit={submit} className="mt-4 rounded-3xl border border-white/[.07] bg-white/[.025] p-4"><textarea value={text} onChange={(e) => setText(e.target.value)} maxLength={5000} rows={4} placeholder="Start a conversation in this community…" className="w-full resize-none bg-transparent text-sm leading-6 text-white outline-none placeholder:text-white/25" /><div className="mt-3 flex justify-end"><button disabled={posting || !text.trim()} className="inline-flex items-center gap-2 rounded-full bg-[#3B82F6] px-4 py-2.5 text-xs font-bold disabled:opacity-50"><FiSend /> {posting ? 'Posting…' : 'Post'}</button></div></form>}
