  ...
  return (
    <section className="mx-auto max-w-3xl px-5 py-12">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div><p className="text-xs font-bold uppercase tracking-[0.25em] text-white/50">Leaders tool</p><h1 className="text-3xl font-extrabold text-white">Push Notifications</h1><p className="mt-1 text-sm text-white/50">Send an announcement to registered BLW Android devices.</p></div>
      </div>
      ...
      <Card as="form" onSubmit={handleSend} variant="custom" className="space-y-4 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
        ...
        <Card variant="custom" className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
          ...
          {targetType !== 'announcement' && <div className="mt-3"><label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Select content</label><select value={targetId} onChange={(e) => setTargetId(e.target.value)} disabled={loadingTargets || busy} className="w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-white outline-none focus:border-gold-500 disabled:opacity-60"><option value="">{loadingTargets ? 'Loading content…' : `Choose ${targetLabel.toLowerCase()}`}</option>{(targetData[targetType] || []).map((item) => { const id = getItemId(item, targetType); return <option key={id} value={id}>{getItemTitle(item, targetType)}</option>; })}</select><p className="mt-1.5 text-xs text-white/50">Choose an existing item so the notification always points to a valid destination.</p></div>}
        </Card>

        <Card variant="custom" className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-white/50"><FiEye /> Preview</div><div className="rounded-2xl border border-white/10 bg-ink-900 p-4"><div className="flex items-start gap-3"><div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-gold-500"><FiBell className="h-4 w-4" /></div><div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-wider text-white/50">BLW Campus Ministry</p><h3 className="mt-1 truncate text-sm font-bold text-white">{title.trim() || 'Notification title'}</h3><p className="mt-1 text-sm leading-5 text-white/55">{body.trim() || 'Your notification message will appear here.'}</p><p className="mt-2 text-[11px] font-semibold text-gold-500">Tap to open: {previewTarget}</p></div></div></div></Card>
        ...
        {!broadcast && <div><label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Member emails</label><textarea value={emails} onChange={(e) => setEmails(e.target.value)} rows={4} placeholder="member@example.com, another@example.com" className="w-full resize-y rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-white outline-none focus:border-gold-500" /></div>}

        <div className="grid gap-3 sm:grid-cols-2">
          <Button variant="secondary" size="none" type="button" onClick={handleSelfTest} disabled={busy || loadingTargets || (targetType !== 'announcement' && !selectedTarget) || !title.trim() || !body.trim()} className="flex w-full items-center justify-center gap-2 rounded-xl py-3 font-bold disabled:opacity-50"><FiUserCheck /> {busy ? 'Sending…' : 'Send Test to Me'}</Button>
          <Button variant="gradient" size="none" type="submit" disabled={busy || loadingTargets || (targetType !== 'announcement' && !selectedTarget)} className="flex w-full items-center justify-center gap-2 rounded-xl py-3 font-bold disabled:opacity-50"><FiSend /> {busy ? 'Sending…' : 'Send Notification'}</Button>
        </div>
      ...
      <Card variant="custom" className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
        <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-white/50">Recent activity</p><h2 className="mt-1 text-xl font-bold text-white">Delivery Log</h2></div><FiClock className="text-white/30" /></div>
        {!deliveryLog.length ? <Card as="p" variant="custom" className="rounded-2xl border border-white/10 bg-white/[0.025] px-4 py-5 text-sm text-white/50">No notifications have been sent from this device yet.</Card> : <div className="space-y-2">{deliveryLog.map((entry, index) => <Card key={`${entry.timestamp}-${index}`} variant="custom" className="rounded-2xl border border-white/10 bg-white/[0.025] px-4 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${entry.mode === 'test' ? 'bg-gold-500/10 text-gold-500' : 'bg-white/10 text-white/50'}`}>{entry.mode === 'test' ? 'Test' : 'Broadcast'}</span><span className="truncate text-sm font-semibold text-white">{entry.target}</span></div><p className="mt-1 text-xs text-white/50">{new Date(entry.timestamp).toLocaleString()}</p></div><span className={`shrink-0 text-xs font-bold ${entry.status === 'sent' ? 'text-emerald-300' : 'text-gold-500'}`}>{entry.status === 'sent' ? `✓ ${entry.sent}/${entry.total}` : '⚠ No devices'}</span></div></Card>)}</div>}
      </Card>
    </section>
  );
}
