...
export default function NotificationCenter() {
  ...
  return (
    <section className="mx-auto max-w-3xl px-5 py-12">
      ...
      <form onSubmit={handleSend} className="space-y-4 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
          ...
          <select value={targetType} onChange={(e) => { setTargetType(e.target.value); setTargetId(''); setError(''); }} className="w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-white outline-none focus:border-[#F2A31C]">{TARGETS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          {targetType !== 'announcement' && <div className="mt-3"><label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-white/40">Select content</label><select value={targetId} onChange={(e) => setTargetId(e.target.value)} disabled={loadingTargets || busy} className="w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-white outline-none focus:border-[#F2A31C] disabled:opacity-60"><option value="">{loadingTargets ? 'Loading content…' : `Choose ${targetLabel.toLowerCase()}`}</option>{(targetData[targetType] || []).map((item) => { const id = getItemId(item, targetType); return <option key={id} value={id}>{getItemTitle(item, targetType)}</option>; })}</select><p className="mt-1.5 text-xs text-white/30">Choose an existing item so the notification always points to a valid destination.</p></div>}
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={handleSelfTest} disabled={busy || loadingTargets || (targetType !== 'announcement' && !selectedTarget) || !title.trim() || !body.trim()} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-[#F2A31C]/30 bg-[#F2A31C]/10 py-3 font-bold text-[#F2A31C] disabled:opacity-50"><FiUserCheck /> {busy ? 'Sending…' : 'Send Test to Me'}</button>
          <button type="submit" disabled={busy || loadingTargets || (targetType !== 'announcement' && !selectedTarget)} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#EC2FA8] via-[#8A2BE2] to-[#3D5AFE] py-3 font-bold text-white disabled:opacity-50"><FiSend /> {busy ? 'Sending…' : 'Send Notification'}</button>
        </div>
      ...
      <div className="mt-8 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
        ...
        {!deliveryLog.length ? <p className="rounded-2xl border border-white/10 bg-white/[0.025] px-4 py-5 text-sm text-white/35">No notifications have been sent from this device yet.</p> : <div className="space-y-2">{deliveryLog.map((entry, index) => <div key={`${entry.timestamp}-${index}`} className="rounded-2xl border border-white/10 bg-white/[0.025] px-4 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${entry.mode === 'test' ? 'bg-[#F2A31C]/10 text-[#F2A31C]' : 'bg-white/10 text-white/50'}`}>{entry.mode === 'test' ? 'Test' : 'Broadcast'}</span><span className="truncate text-sm font-semibold text-white">{entry.target}</span></div><p className="mt-1 text-xs text-white/35">{new Date(entry.timestamp).toLocaleString()}</p></div><span className={`shrink-0 text-xs font-bold ${entry.status === 'sent' ? 'text-emerald-300' : 'text-[#F2A31C]'}`}>{entry.status === 'sent' ? `✓ ${entry.sent}/${entry.total}` : '⚠ No devices'}</span></div></div>)}</div>}
      </div>
    </section>
  );
}
