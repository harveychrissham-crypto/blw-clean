...
import { FiBell, FiSend, FiAlertCircle, FiCheckCircle } from 'react-icons/fi';
...
export default function NotificationCenter() {
  ...
  const sendNotification = async (event) => {
    ...
    try {
      ...
      if (targetType !== 'announcement' && !targetId.trim()) throw new Error('Enter the ID of the content this notification should open.');
      const response = await apiFetch('/api/push/send', {
        ...
        body: JSON.stringify({
          ...
          data: { type: targetType, id: targetType === 'announcement' ? '' : targetId.trim(), source: 'leader_notification_center' },
        }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || `Notification send failed (${response.status}).`);
      setStatus(`Notification sent. ${data.sent || 0} device(s) reached.`); setTitle(''); setBody(''); setTargetId('');
  ...
  return (
    <section className="mx-auto max-w-2xl px-5 py-12">
      ...
      <form onSubmit={sendNotification} className="space-y-4 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
        ...
        <div><label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-white/40">Open when tapped</label><select value={targetType} onChange={(e) => { setTargetType(e.target.value); setTargetId(''); }} className="w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-white outline-none focus:border-[#F2A31C]">{TARGETS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
        {targetType !== 'announcement' && <div><label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-white/40">Content ID</label><input value={targetId} onChange={(e) => setTargetId(e.target.value)} placeholder={targetType === 'venue' ? 'Chapter name' : 'Database ID'} className="w-full rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-white outline-none focus:border-[#F2A31C]" /><p className="mt-1.5 text-xs text-white/30">The app will use this value to identify the destination content.</p></div>}
        ...
        <button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#EC2FA8] via-[#8A2BE2] to-[#3D5AFE] py-3 font-bold text-white disabled:opacity-50"><FiSend /> {busy ? 'Sending…' : 'Send Notification'}</button>
      </form>
    </section>
  );
}
