...
function LeadershipToolsCard({ onOpen }) {
  return (
    <div onClick={onOpen} role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onOpen()} className="group cursor-pointer overflow-hidden rounded-[2rem] border border-[#8A2BE2]/25 bg-[#11101a] p-6 transition hover:-translate-y-0.5 hover:border-[#EC2FA8]/40 hover:bg-[#161522]">
      <div className="flex items-center justify-between gap-5">
        ...
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#181624] text-[#D8B2FF] transition group-hover:bg-[#201c30]"><FiTool className="h-7 w-7" /></div>
      </div>
    </div>
  );
}
  ...
  return createPortal(
    <div className="fixed inset-0 z-[110] overflow-y-auto bg-[#0d0c18]/98 backdrop-blur-xl">
      <div className="min-h-screen px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto max-w-5xl">
        <div className="mb-8 flex items-start justify-between gap-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#F2A31C]">Leaders Forum</p><h2 className="mt-2 text-3xl font-extrabold text-white">Leadership Tools</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/45">All management tools are organized here so the main Leaders Forum stays focused on attendance and the member directory.</p></div><button onClick={onClose} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-white/10 bg-[#11101a] px-4 py-2 text-sm font-semibold text-white/75 hover:bg-[#181722]"><FiX /> Back to Leaders Forum</button></div>
        <div className="space-y-8">
          <section><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#D8B2FF]">Content Management</p><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{TOOL_BUTTONS.filter((tool) => ['Manage Events','Manage Outreach','Manage Sermons'].includes(tool.label)).map((tool) => { const Icon=tool.icon; return <button key={tool.label} onClick={() => openInternal(tool.label)} className="group rounded-2xl border border-white/10 bg-[#101017] p-5 text-left transition hover:-translate-y-0.5 hover:border-white/15 hover:bg-[#15151e]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">{tool.label}</h3><p className="mt-1 text-xs leading-relaxed text-white/40">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-[#A53DFF]" /></div></button>; })}</div></section>
          <section><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#D8B2FF]">Campus & Fellowship</p><div className="grid gap-3 sm:grid-cols-2">{TOOL_BUTTONS.filter((tool) => ['Manage Fellowship Locations','Manage Service Venues'].includes(tool.label)).map((tool) => { const Icon=tool.icon; const action=tool.kind==='fellowship'?() => { onClose(); onFellowship(); }:() => openInternal(tool.label); return <button key={tool.label} onClick={action} className="group rounded-2xl border border-white/10 bg-[#101017] p-5 text-left transition hover:-translate-y-0.5 hover:border-white/15 hover:bg-[#15151e]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">{tool.label}</h3><p className="mt-1 text-xs leading-relaxed text-white/40">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-[#D8B2FF]" /></div></button>; })}</div></section>
          <section><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#D8B2FF]">Broadcast</p><div className="grid gap-3 sm:grid-cols-2">{TOOL_BUTTONS.filter((tool) => tool.label==='Manage Live Stream').map((tool) => { const Icon=tool.icon; return <button key={tool.label} onClick={() => openInternal(tool.label)} className="group rounded-2xl border border-white/10 bg-[#101017] p-5 text-left transition hover:-translate-y-0.5 hover:border-white/15 hover:bg-[#15151e]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">{tool.label}</h3><p className="mt-1 text-xs leading-relaxed text-white/40">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-red-300" /></div></button>; })}</div></section>
        </div>
      </div></div>
    </div>, document.body,
  );
...
export default function LeadersForumWithFellowship() {
  ...
  return <>
    ...
    {showFellowship&&<div className="fixed inset-0 z-[120] overflow-y-auto bg-[#0d0c18]/98 backdrop-blur-xl"><div className="min-h-screen px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="mb-3 flex justify-end"><button type="button" onClick={()=>{setShowFellowship(false);setToolsOpen(true);}} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#11101a] px-4 py-2 text-sm font-semibold text-white/80 hover:bg-[#181722]"><FiX /> Back to Leadership Tools</button></div><FellowshipLocationsAdmin /></div></div></div>}
  </>;
}
