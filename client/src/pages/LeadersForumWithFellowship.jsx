  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-5">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.4em] text-[#D8B2FF]">Leadership</p>
          <h3 className="mt-2 text-xl font-bold text-white">Leadership Tools ▸</h3>
          <p className="mt-1 text-sm text-white/50">Manage content, campus fellowships, venues, and the live stream from one place.</p>
        </div>
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#8A2BE2]/20 text-[#D8B2FF] transition group-hover:bg-[#8A2BE2]/30">
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-8 flex items-start justify-between gap-5">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-extrabold text-white">Leadership Tools</h2>
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/45">All management tools are organized here so the main Leaders Forum stays focused on attendance and the member directory.</p>
            </div>
            <button onClick={onClose} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-semibold text-white/75 hover:bg-white/10">
              <FiX /> Back to Leaders Forum
            </button>
          </div>

          <div className="space-y-8">
            <section>
              <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#D8B2FF]">Content Management</p>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {TOOL_BUTTONS.filter((tool) => ['Manage Events', 'Manage Outreach', 'Manage Sermons'].includes(tool.label)).map((tool) => {
                  const Icon = tool.icon;
                  return <button key={tool.label} onClick={() => openInternal(tool.label)} className="group rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-left transition hover:-translate-y-0.5 hover:bg-white/[0.07]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">{tool.label}</h3><p className="mt-1 text-xs leading-relaxed text-white/40">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-[#A53DFF]" /></div></button>;
                })}
              </div>
            </section>

            <section>
              <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#D8B2FF]">Campus & Fellowship</p>
              <div className="grid gap-3 sm:grid-cols-2">
                {TOOL_BUTTONS.filter((tool) => ['Manage Fellowship Locations', 'Manage Service Venues'].includes(tool.label)).map((tool) => {
                  const Icon = tool.icon;
                  const action = tool.kind === 'fellowship' ? () => { onClose(); onFellowship(); } : () => openInternal(tool.label);
                  return <button key={tool.label} onClick={action} className="group rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-left transition hover:-translate-y-0.5 hover:bg-white/[0.07]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">{tool.label}</h3><p className="mt-1 text-xs leading-relaxed text-white/40">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-[#D8B2FF]" /></div></button>;
                })}
              </div>
            </section>

            <section>
              <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#D8B2FF]">Broadcast</p>
              <div className="grid gap-3 sm:grid-cols-2">
                {TOOL_BUTTONS.filter((tool) => tool.label === 'Manage Live Stream').map((tool) => {
                  const Icon = tool.icon;
                  return <button key={tool.label} onClick={() => openInternal(tool.label)} className="group rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-left transition hover:-translate-y-0.5 hover:bg-white/[0.07]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">{tool.label}</h3><p className="mt-1 text-xs leading-relaxed text-white/40">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-red-300" /></div></button>;
                })}
              </div>
...
export default function LeadersForumWithFellowship() {
    ...
    const findToolsGrid = () => {
      ...
      if (grid) {
        ...
        cards.forEach((card) => {
          ...
          if (['Manage Events', 'Manage Outreach', 'Manage Sermons', 'Manage Service Venues', 'Manage Live Stream'].some((label) => text.includes(label))) {
  ...
  return (
    <>
      ...
      {showFellowship && (
        <div className="fixed inset-0 z-[120] overflow-y-auto bg-[#0d0c18]/98 backdrop-blur-xl">
          <div className="min-h-screen px-4 py-6 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
              <div className="mb-3 flex justify-end">
                <button type="button" onClick={() => setShowFellowship(false)} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-semibold text-white/80 hover:bg-white/10">
                  <FiX /> Back to Leadership Tools
                </button>
              </div>
