...
function CSVDownload({ members }) {
  const exportCsv = () => {
    ...
    anchor.href = url;
    anchor.download = `blw-attendance-${date}.csv`;
    anchor.click();
...
function QRScannerPanel({ onCheckIn, onClose, checkedInCount, pendingCount }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-[#0d0c18]/95 px-4 py-6">
    <Card variant="raised" className="relative w-full max-w-lg rounded-[2rem] shadow-2xl">
      ...
      {!result && <div className="flex border-b border-white/10"><button onClick={() => { setError(''); setManual(''); }} className="flex flex-1 items-center justify-center gap-2 border-b-2 border-[#F2A31C] py-3 text-sm font-semibold text-[#F2A31C]"><MdQrCodeScanner/> Scan QR</button><button onClick={() => { stopCamera(); setError(''); setManual(''); }} className="flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold text-white/45"><FiSearch/> Manual</button></div>}
      ...
      <div className="max-h-[78vh] overflow-y-auto p-5">
        {!result && !manual && <>
          ...
          <div className="mt-4 flex gap-2"><button onClick={() => void startCamera()} className="flex-1 rounded-2xl bg-[#F2A31C] py-3 text-sm font-bold text-[#0d0c18]">{scanning ? 'Restart camera' : 'Start camera'}</button>{scanning && torchAvailable && <button onClick={() => void toggleTorch()} className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${torchOn ? 'border-[#F2A31C] text-[#F2A31C]' : 'border-white/10 text-white/70'}`} aria-label="Toggle flashlight"><MdFlashlightOn/></button>}</div>
...
function AttendancePanel() {
  ...
  useEffect(() => {
    ...
    const onOnline = async () => {
      try { await syncOfflineCheckins(); await refreshMembers(); } catch { /* banner handles connectivity */ }
      await refreshQueue();
    };
  ...
  return <>
    <Card variant="raised" className="p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><FiUsers className="text-emerald-300"/><div><Eyebrow>Attendance</Eyebrow><h2 className="text-xl font-bold text-white">Member check-in</h2><p className="text-xs text-white/40">{checkedInCount} checked in today</p></div></div><div className="flex gap-2"><CSVDownload members={members}/><button onClick={() => { setError(''); setScannerOpen(true); }} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#F2A31C] px-4 py-3 text-sm font-bold text-[#0d0c18]"><MdQrCodeScanner className="h-5 w-5"/> Scan QR</button></div></div>
