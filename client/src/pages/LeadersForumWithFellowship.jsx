  ...
  return (
    <section className="mx-auto max-w-xl px-5 py-20 text-center">
      <Card variant="raised" className="p-8">
        ...
        <button onClick={() => navigate('/auth')} className="mt-6 rounded-full bg-white/10 px-5 py-3 text-sm font-semibold text-white hover:bg-white/15">Sign in with an authorized account</button>
      </Card>
    </section>
  );
}
...
function AttendancePanel() {
  ...
  useEffect(() => {
    fetchAllMembers().then(setMembers).catch((e) => setError(e.message || 'Unable to load members.')).finally(() => setLoading(false));
  }, []);
  ...
  const checkIn = async (membershipId) => {
    setSaving(membershipId); setError('');
    try {
      const updated = await checkInMember(membershipId);
      setMembers((current) => current.map((m) => m.membershipId === membershipId ? { ...m, ...updated } : m));
    } catch (e) {
  ...
  return (
    <Card variant="raised" className="p-5">
      <div className="flex items-center gap-3"><FiUsers className="text-emerald-300" /><div><Eyebrow>Attendance</Eyebrow><h2 className="text-xl font-bold text-white">Member check-in</h2></div></div>
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-12">
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><Eyebrow>Leaders Forum</Eyebrow><h1 className="mt-2 text-3xl font-extrabold text-white">Leadership Tools</h1><p className="mt-2 max-w-3xl text-sm text-white/45">Administrator-only management for attendance, content, fellowship locations, live streaming, and notifications.</p></div>
      ...
      {view === 'fellowship' && <><button onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</button><FellowshipLocationsAdmin /></>}
