...
import { FiBell, FiCalendar, FiCheckCircle, FiFilm, FiImage, FiMapPin, FiRadio, FiTool, FiX } from 'react-icons/fi';
...
const TOOL_BUTTONS = [
  ...
  { label: 'Push Notifications', description: 'Send announcements to registered BLW Android devices.', kind: 'notifications', icon: FiBell },
];
  ...
  return (
    <Card onClick={onOpen} role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onOpen()} variant="raised" className="group cursor-pointer border-[#8A2BE2]/30 bg-gradient-to-r from-[#8A2BE2]/15 via-[#EC2FA8]/10 to-[#3D5AFE]/10 p-6 transition hover:-translate-y-0.5 hover:border-[#EC2FA8]/50 hover:bg-white/[0.06]">
      <div className="flex items-center justify-between gap-5">
        <div><Eyebrow color="#D8B2FF" className="tracking-[0.4em]">Leadership</Eyebrow><h3 className="mt-2 text-xl font-bold text-white">Leadership Tools ▸</h3><p className="mt-1 text-sm text-white/50">Manage content, campus fellowships, venues, attendance, broadcasts, and notifications from one place.</p></div>
...
function ToolsDashboard({ onClose, onFellowship, onInternalOpen, onAttendance, onNotifications }) {
  return createPortal(
    <div data-leadership-tools-overlay className="fixed inset-0 z-[110] overflow-y-auto overscroll-contain bg-[#0d0c18]">
      <div className="min-h-screen px-4 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:px-6 lg:px-8"><div className="mx-auto max-w-5xl">
        ...
        <div className="space-y-8">
          <section><Eyebrow color="#D8B2FF" className="mb-3">Attendance</Eyebrow><div className="grid gap-3"><Card as="button" onClick={onAttendance} variant="subtle" className="group inline-flex w-fit items-center gap-2 border-white/10 bg-white/[0.05] p-2 text-left transition hover:-translate-y-0.5 hover:bg-white/10"><div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white/10"><FiCheckCircle className="h-3.5 w-3.5 text-slate-300" /></div><div className="min-w-0"><h3 className="text-xs font-bold text-white">Check Attendance</h3><p className="mt-0.5 text-[10px] leading-snug text-white/50">View signed-in members and offline attendance waiting to sync.</p></div></Card></div></section>
          ...
          <section><Eyebrow color="#D8B2FF" className="mb-3">Notifications</Eyebrow><div className="grid gap-3"><Card as="button" onClick={onNotifications} variant="subtle" className="group p-5 text-left transition hover:-translate-y-0.5 hover:bg-white/[0.05]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-white">Push Notifications</h3><p className="mt-1 text-xs leading-relaxed text-white/60">Send announcements to all registered Android devices or selected members.</p></div><FiBell className="h-6 w-6 shrink-0 text-[#F2A31C]" /></div></Card></div></section>
        </div>
      </div></div>
    </div>, document.body,
  );
...
export default function LeadersForumWithFellowship() {
  const [toolsGrid, setToolsGrid] = useState(null), [toolsOpen, setToolsOpen] = useState(false), [showFellowship, setShowFellowship] = useState(false), [showAttendance, setShowAttendance] = useState(false), [showNotifications, setShowNotifications] = useState(false);
  ...
  return <>
    ...
    {toolsOpen&&<ToolsDashboard onClose={()=>setToolsOpen(false)} onFellowship={()=>setShowFellowship(true)} onInternalOpen={openInternal} onAttendance={openAttendance} onNotifications={openNotifications}/>} 
    ...
    {showNotifications&&<div className="fixed inset-0 z-[122] overflow-y-auto overscroll-contain bg-[#0d0c18]"><div className="min-h-screen px-4 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:px-6 lg:px-8"><div className="mx-auto max-w-5xl"><div className="mb-3 flex justify-end"><button type="button" onClick={backToLeadershipTools} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-semibold text-white/80 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiX /> Back to Leadership Tools</button></div><NotificationCenter /></div></div></div>}
    {showFellowship&&<div className="fixed inset-0 z-[121] overflow-y-auto overscroll-contain bg-[#0d0c18]"><div className="min-h-screen px-4 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="mb-3 flex justify-end"><button type="button" onClick={()=>{setShowFellowship(false);setToolsOpen(true);}} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-semibold text-white/80 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><FiX /> Back to Leadership Tools</button></div><FellowshipLocationsAdmin /></div></div></div>}
  </>;
}
