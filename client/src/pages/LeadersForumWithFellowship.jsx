  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-ink-900/95 px-4 py-6">
    <Card variant="raised" className="relative w-full max-w-lg rounded-[2rem] shadow-2xl">
      ...
      <div className="flex border-b border-white/10">
        <Button variant="custom" size="none" onClick={() => { setTab('scan'); setResult(null); setError(''); }} className={`flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold transition ${tab === 'scan' ? 'border-b-2 border-gold-500 text-gold-500' : 'text-white/45'}`}><MdQrCodeScanner/> Scan QR</Button>
        <Button variant="custom" size="none" onClick={() => { stopCamera(); setTab('manual'); setResult(null); setError(''); }} className={`flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold transition ${tab === 'manual' ? 'border-b-2 border-gold-500 text-gold-500' : 'text-white/45'}`}><FiSearch/> Manual</Button>
      </div>
      <div className="max-h-[78vh] overflow-y-auto p-5">
        {!result && tab === 'scan' && <>
          ...
          <div className="mt-4 flex gap-2"><Button variant="custom" size="none" onClick={() => void startCamera()} className="flex-1 rounded-2xl bg-gold-500 py-3 text-sm font-bold text-ink-900">{scanning ? 'Restart camera' : 'Start camera'}</Button>{scanning && torchAvailable && <Button variant="custom" size="none" onClick={() => void toggleTorch()} className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${torchOn ? 'border-gold-500 text-gold-500' : 'border-white/10 text-white/70'}`} aria-label="Toggle flashlight"><MdFlashlightOn/></Button>}</div>
...
function AttendancePanel() {
  ...
  return <>
    <Card variant="raised" className="p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><FiUsers className="text-emerald-300"/><div><Eyebrow>Attendance</Eyebrow><h2 className="text-xl font-bold text-white">Member check-in</h2><p className="text-xs text-white/50">{selectedEvent ? `${selectedEvent.title} · ${selectedEvent.date || 'No date'}` : 'Select an event'} · {checkedInCount} checked in</p></div></div><div className="flex gap-2"><CSVDownload members={members} event={selectedEvent}/><Button variant="custom" size="none" disabled={!selectedEventId} onClick={() => { setError(''); setScannerOpen(true); }} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gold-500 px-4 py-3 text-sm font-bold text-ink-900 disabled:cursor-not-allowed disabled:opacity-50"><MdQrCodeScanner className="h-5 w-5"/> Scan QR</Button></div></div>
