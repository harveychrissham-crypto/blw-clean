    ...
    <Card variant="raised" className="relative w-full max-w-lg rounded-[2rem] shadow-2xl">
      ...
      <div className="flex border-b border-white/10">
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold transition ${tab === 'scan' ? 'border-b-2 border-[#F2A31C] text-[#F2A31C]' : 'text-white/45'}`}
        >
          <MdQrCodeScanner/> Scan QR
        </button>
        <button
          onClick={() => { stopCamera(); setTab('manual'); setResult(null); setError(''); }}
          className={`flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold transition ${tab === 'manual' ? 'border-b-2 border-[#F2A31C] text-[#F2A31C]' : 'text-white/45'}`}
        >
          <FiSearch/> Manual
        </button>
      </div>
