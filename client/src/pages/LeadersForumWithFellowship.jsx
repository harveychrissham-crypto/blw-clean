...
function CSVDownload({ members, event }) {
  ...
  return <Button variant="custom" size="none" onClick={exportCsv} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-semibold text-white/70 hover:bg-white/[0.08]"><FiDownload/> CSV</Button>;
}
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-ink-900/95 px-4 py-6">
    <Card variant="raised" className="relative w-full max-w-lg rounded-[2rem] shadow-2xl">
      <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
        <div><Eyebrow>Leaders tool</Eyebrow><h2 className="mt-1 text-xl font-bold text-white">Member Check-In</h2><p className="mt-1 text-xs text-white/50">{event?.title || 'Selected event'}{event?.date ? ` · ${event.date}` : ''} · {checkedInCount} checked in{pendingCount ? ` · ${pendingCount} pending sync` : ''}</p></div>
        <IconButton onClick={onClose} aria-label="Close scanner"><FiX/></IconButton>
      </div>
      <div className="flex border-b border-white/10">
        <Button variant="custom" size="none" onClick={() => { setTab('scan'); setResult(null); setError(''); }} className={`flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold transition ${tab === 'scan' ? 'border-b-2 border-[#EC2FA8] text-[#EC2FA8]' : 'text-white/45'}`}><MdQrCodeScanner/> Scan QR</Button>
        <Button variant="custom" size="none" onClick={() => { stopCamera(); setTab('manual'); setResult(null); setError(''); }} className={`flex flex-1 items-center justify-center gap-2 py-3 text-sm font-semibold transition ${tab === 'manual' ? 'border-b-2 border-[#EC2FA8] text-[#EC2FA8]' : 'text-white/45'}`}><FiSearch/> Manual</Button>
      </div>
      <div className="max-h-[78vh] overflow-y-auto p-5">
        {!result && tab === 'scan' && <>
          <div className="relative aspect-square overflow-hidden rounded-2xl bg-black/50"><video ref={videoRef} className="h-full w-full object-cover" muted playsInline/><canvas ref={canvasRef} className="hidden"/>{scanning && <div className="pointer-events-none absolute inset-0 flex items-center justify-center"><div className="h-52 w-52 rounded-2xl border-2 border-[#EC2FA8]/70"/></div>}{!scanning && <div className="absolute inset-0 flex flex-col items-center justify-center gap-3"><MdQrCodeScanner className="h-14 w-14 text-white/25"/><p className="text-sm text-white/50">Camera inactive</p></div>}</div>
          <div className="mt-4 flex gap-2"><Button variant="primary" size="none" onClick={() => void startCamera()} className="flex-1 rounded-2xl py-3 text-sm">{scanning ? 'Restart camera' : 'Start camera'}</Button>{scanning && torchAvailable && <Button variant="custom" size="none" onClick={() => void toggleTorch()} className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${torchOn ? 'border-[#EC2FA8] text-[#EC2FA8]' : 'border-white/10 text-white/70'}`} aria-label="Toggle flashlight"><MdFlashlightOn/></Button>}</div>
          <div className="mt-3 flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3"><span className="text-xs text-white/55">Continuous scan</span><Button variant="custom" size="none" onClick={() => setContinuous((v) => !v)} className={`h-6 w-11 rounded-full p-1 transition ${continuous ? 'bg-emerald-500' : 'bg-white/15'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${continuous ? 'translate-x-5' : ''}`}/></Button></div>
          <p className="mt-3 text-center text-xs text-white/50">Point the camera at the member's QR badge.</p>
        </>}
        {!result && tab === 'manual' && <div className="mt-5"><label className="text-sm font-semibold text-white">Manual lookup</label><div className="mt-2 flex gap-2"><input value={manual} onChange={handleManualChange} onKeyDown={(e) => { if (e.key === 'Enter') void lookupManual(); }} placeholder="Membership ID, name, phone, or email" className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm text-white outline-none"/><Button variant="primary" size="none" onClick={() => void lookupManual()} disabled={loading} className="rounded-2xl px-4 py-3 text-sm">Find</Button></div>{suggestions.length > 0 && <div className="mt-2 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">{suggestions.map((member) => <Button variant="custom" size="none" key={member.membershipId} onClick={() => { setResult({ member, confirmed: false }); setSuggestions([]); }} className="block w-full border-b border-white/5 px-4 py-3 text-left last:border-0 hover:bg-white/[0.05]"><p className="text-sm font-semibold text-white">{member.name}</p><p className="text-xs text-white/50">{member.membershipId}{member.chapter ? ` · ${member.chapter}` : ''}</p></Button>)}</div>}</div>}
        {error && <p className="mt-4 rounded-xl bg-red-500/10 px-4 py-3 text-sm text-red-300">{error}</p>}
        {result?.member && <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.04] p-5"><Eyebrow>Member found</Eyebrow><h3 className="mt-2 text-2xl font-bold text-white">{result.member.name}</h3><p className="mt-1 text-sm text-white/45">{result.member.membershipId}{result.member.chapter ? ` · ${result.member.chapter}` : ''}</p>{result.already || result.member.checkedIn ? <div className="mt-5 rounded-2xl bg-amber-500/10 px-4 py-4 text-sm font-semibold text-amber-300">Already checked in for this event{result.member.checkedInAt ? ` at ${new Date(result.member.checkedInAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}.</div> : result.confirmed ? <div className="mt-5 rounded-2xl bg-emerald-500/10 px-4 py-4 text-sm font-semibold text-emerald-300">Checked in successfully.</div> : <Button variant="custom" size="none" onClick={() => void confirmCheckIn()} disabled={loading} className="mt-5 w-full rounded-2xl bg-emerald-500/20 py-3 text-sm font-bold text-emerald-200 disabled:opacity-50">{loading ? 'Checking in…' : 'Confirm Check-In'}</Button>} {!continuous && <Button variant="custom" size="none" onClick={() => { setResult(null); setManual(''); setError(''); }} className="mt-3 w-full rounded-2xl border border-white/10 py-3 text-sm font-semibold text-white/60">Scan another member</Button>}</div>}
      </div>
    </Card>
  </div>;
}
...
function AttendancePanel() {
  ...
  return <>
    <Card variant="raised" className="p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><FiUsers className="text-emerald-300"/><div><Eyebrow>Attendance</Eyebrow><h2 className="text-xl font-bold text-white">Member check-in</h2><p className="text-xs text-white/50">{selectedEvent ? `${selectedEvent.title} · ${selectedEvent.date || 'No date'}` : 'Select an event'} · {checkedInCount} checked in</p></div></div><div className="flex gap-2"><CSVDownload members={members} event={selectedEvent}/><Button variant="primary" size="none" disabled={!selectedEventId} onClick={() => { setError(''); setScannerOpen(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm"><MdQrCodeScanner className="h-5 w-5"/> Scan QR</Button></div></div>
      <div className="mt-5"><label className="text-xs font-semibold uppercase tracking-wider text-white/50">Attendance event</label><select value={selectedEventId} onChange={(e) => setSelectedEventId(e.target.value)} disabled={eventsLoading} className="mt-2 w-full rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm text-white outline-none disabled:opacity-50"><option value="">{eventsLoading ? 'Loading events…' : 'Select an event'}</option>{events.map((event) => <option key={event.id} value={event.id}>{event.title}{event.date ? ` · ${event.date}` : ''}{event.time ? ` · ${event.time}` : ''}</option>)}</select></div>
...
export default function LeadersForumWithFellowship() {
  ...
  return <section className="mx-auto max-w-6xl px-5 py-12"><div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><Eyebrow>Leaders Forum</Eyebrow><h1 className="mt-2 text-3xl font-extrabold text-white">Leadership Tools</h1><p className="mt-2 max-w-3xl text-sm text-white/45">Administrator-only management for attendance, content, fellowship locations, live streaming, and notifications.</p></div><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="inline-flex items-center gap-2 text-sm text-white/50 hover:text-white"><FiLock/> Admin session</Button></div>{view === 'dashboard' && <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{tools.map((tool) => { const Icon = tool.icon; return <Card as="button" key={tool.label} onClick={() => setView(tool.path)} variant="raised" className="p-5 text-left transition hover:-translate-y-0.5 hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-bold text-white">{tool.label}</h2><p className="mt-2 text-sm leading-relaxed text-white/45">{tool.description}</p></div><Icon className="h-6 w-6 shrink-0 text-purple-300"/></div></Card>; })}</div>}{view === 'attendance' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><AttendancePanel/></>}{view === 'fellowship' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><FellowshipLocationsAdminSecure/></>}{view === 'notifications' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><NotificationCenter/></>}{view === 'live' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><LiveStreamAdminPanel/></>}{view === 'admin-events' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><ContentAdminPanel initialTab="events" onClose={() => setView('dashboard')}/></>}{view === 'admin-outreach' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><ContentAdminPanel initialTab="outreach" onClose={() => setView('dashboard')}/></>}{view === 'admin-sermons' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><ContentAdminPanel initialTab="sermons" onClose={() => setView('dashboard')}/></>}{view === 'admin-venues' && <><Button variant="custom" size="none" onClick={() => setView('dashboard')} className="mb-4 text-sm text-white/50 hover:text-white">← Back</Button><ContentAdminPanel initialTab="venues" onClose={() => setView('dashboard')}/></>}</section>;
}
