...
function AttendancePanel() {
  ...
  return <>
    <Card variant="raised" className="p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><FiUsers className="text-emerald-300"/><div><Eyebrow>Attendance</Eyebrow><h2 className="text-xl font-bold text-white">Member check-in</h2><p className="text-xs text-white/40">{selectedEvent ? `${selectedEvent.title} · ${selectedEvent.date || 'No date'}` : 'Select an event'} · {checkedInCount} checked in</p></div></div><div className="flex gap-2"><CSVDownload members={members} event={selectedEvent}/><Button variant="custom" size="none" disabled={!selectedEventId} onClick={() => { setError(''); setScannerOpen(true); }} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gold-500 px-4 py-3 text-sm font-bold text-ink-900 disabled:cursor-not-allowed disabled:opacity-50"><MdQrCodeScanner className="h-5 w-5"/> Scan QR</Button></div></div>
