    ...
    <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <Card variant="raised" className="p-5"><div className="flex items-center justify-between"><div><Eyebrow>{editingId ? 'Edit' : 'Create'}</Eyebrow><h3 className="mt-1 text-xl font-bold text-white">{editingId ? `Edit ${tabTitle}` : `New ${tabTitle}`}</h3></div>{editingId && <IconButton onClick={reset} aria-label="Cancel edit"><FiX/></IconButton>}</div>
        <form onSubmit={submit} className="mt-5 space-y-4">
          ...
          {tab === 'venues' && <><Field label="Chapter" value={form.chapter} onChange={(v) => setField('chapter', v)} disabled={Boolean(editingId)} /><p className="-mt-2 text-xs text-white/50">{editingId ? 'Chapter is locked while editing because it is the venue key.' : 'Choose the chapter name carefully; it is used as the venue key.'}</p><Field label="Venue" value={form.venue} onChange={(v) => setField('venue', v)} /><Field label="Service time" value={form.serviceTime} onChange={(v) => setField('serviceTime', v)} /></>}
          <Button variant="custom" size="none" type="submit" disabled={saving || uploading} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-pink-600 via-purple-500 to-indigo-500 px-5 py-3 text-sm font-bold text-white disabled:opacity-50"><FiSave/> {saving ? 'Saving...' : editingId ? 'Save Changes' : 'Create'}</Button>
        </form>
      </Card>
