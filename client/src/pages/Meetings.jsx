...
function CallRoom({ credentials, choices, room: roomName, onLeave }) {
  ...
  async function toggleMic() {
    try {
      ...
      setMic(next);
      if (!next) { setShowMutedHint(true); clearTimeout(mutedHintTimer.current); mutedHintTimer.current = setTimeout(() => setShowMutedHint(false), 3000); }
      else { setShowMutedHint(false); clearTimeout(mutedHintTimer.current); }
    } catch (e) { setError(e.message); }
  }
  ...
  async function toggleBlur() {
    ...
    try {
      if (blurOn) { await track.stopProcessor(); setBlurOn(false); }
      else {
    ...
    } catch (e) { setError(e?.message || 'Unable to change background blur. Your device may not support it.'); }
    finally { setBlurBusy(false); }
  }
    ...
    <section ref={containerRef} className={`mx-auto max-w-7xl px-2 pb-24 pt-3 sm:px-4 ${isFullscreen ? 'flex h-screen max-w-none flex-col justify-center overflow-y-auto bg-ink-900' : ''}`}>
      <div className="flex gap-3">
        <div className="min-w-0 flex-1">
          <div className="mb-3 flex items-center justify-between gap-4 px-1"><div className="min-w-0"><p className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-gold-500">Live room{recording && <span className="inline-flex items-center gap-1.5 rounded-full bg-red-500/15 px-2 py-0.5 text-[10px] font-semibold normal-case tracking-normal text-red-300"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-red-400" aria-hidden="true" />Recording</span>}</p><h1 className="truncate text-xl font-bold sm:text-2xl">{roomName}</h1><ConnectionStateBadge connState={connState} connected={connected} count={all.length} /></div><Button variant="custom" size="none" onClick={leaveVoluntarily} className="hidden shrink-0 items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm sm:inline-flex"><FiLogOut/>Leave</Button></div>
          {connState === 'reconnecting' && <div className="mb-3 flex items-center gap-2 rounded-2xl border border-amber-400/20 bg-amber-400/5 px-4 py-2.5 text-sm text-amber-200"><span className="h-2 w-2 animate-pulse rounded-full bg-amber-300" /> Reconnecting — hang on, trying to get you back into the meeting…</div>}
          {gridArea}
          <ReactionsBar liveRoom={roomRef.current} localParticipant={local} participants={participants} />
          {showMutedHint && <div className="fixed bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-full border border-white/10 bg-[#202124]/95 px-4 py-1.5 text-xs text-white/80 shadow-lg backdrop-blur">🔇 You are muted</div>}
          <div className="fixed inset-x-0 bottom-0 z-50 flex justify-center px-2 pb-[max(0.65rem,env(safe-area-inset-bottom))] sm:px-4 sm:pb-4"><div className="flex w-fit max-w-[calc(100vw-1rem)] items-center gap-1.5 overflow-x-auto rounded-full border border-white/10 bg-[#111216]/95 p-1.5 shadow-[0_18px_60px_rgba(0,0,0,.55)] backdrop-blur-2xl sm:gap-2 sm:p-2"><ControlButton active={mic} onClick={toggleMic} onIcon={FiMic} offIcon={FiMicOff} label="Microphone"/><ControlButton active={camera} onClick={toggleCamera} onIcon={FiCamera} offIcon={FiCameraOff} label="Camera"/><ControlButton active={false} onClick={() => window.dispatchEvent(new Event('blw-meeting-raise-hand'))} onIcon={FiUsers} offIcon={FiUsers} label="Raise hand"/><ControlButton active={false} onClick={() => window.dispatchEvent(new Event('blw-meeting-toggle-reactions'))} onIcon={FiMessageSquare} offIcon={FiMessageSquare} label="Reactions"/><ControlButton active={sharing} onClick={toggleShare} onIcon={FiMonitor} offIcon={FiMonitor} label={screenShareSupported ? 'Present' : 'Present not supported'} disabled={!screenShareSupported}/><ControlButton active={showChat} onClick={openChat} onIcon={FiMessageSquare} offIcon={FiMessageSquare} label="Chat"/>{isHost && <ControlButton active={showHost} onClick={openHost} onIcon={FiUsers} offIcon={FiUsers} label="Participants"/>}<div className="mx-0.5 h-7 w-px shrink-0 bg-white/10 sm:mx-1" aria-hidden="true"/><div className="relative shrink-0"><ControlButton active={showMore} onClick={openMore} onIcon={FiMoreHorizontal} offIcon={FiMoreHorizontal} label="More options"/>{showMore && <><Button variant="custom" size="none" type="button" aria-label="Close menu" onClick={() => setShowMore(false)} className="fixed inset-0 z-30 cursor-default !bg-transparent !border-0 !p-0"/><Card variant="custom" className="absolute bottom-full right-0 z-40 mb-3 w-60 overflow-hidden rounded-2xl border border-white/10 bg-[#1a1926] shadow-2xl"><Button variant="custom" size="none" type="button" onClick={() => { toggleShare(); setShowMore(false); }} disabled={!screenShareSupported} className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-white/85 hover:bg-white/5 disabled:text-white/30"><FiMonitor className="h-4 w-4 shrink-0"/><span className="flex-1">{screenShareSupported ? (sharing ? 'Stop presenting' : 'Present screen, window or tab') : 'Present not supported'}</span>{sharing && <FiCheck className="h-4 w-4 shrink-0 text-amber-300"/>}</Button><Button variant="custom" size="none" type="button" onClick={() => { toggleBlur(); setShowMore(false); }} disabled={blurBusy || !camera} className="flex w-full items-center gap-3 border-t border-white/5 px-4 py-3 text-left text-sm text-white/85 hover:bg-white/5 disabled:text-white/30"><FiDroplet className="h-4 w-4 shrink-0"/><span className="flex-1">{blurBusy ? 'Loading blur…' : camera ? 'Blur my background' : 'Turn camera on to blur'}</span>{blurOn && <FiCheck className="h-4 w-4 shrink-0 text-amber-300"/>}</Button><Button variant="custom" size="none" type="button" onClick={() => { toggleFullscreen(); setShowMore(false); }} className="flex w-full items-center gap-3 border-t border-white/5 px-4 py-3 text-left text-sm text-white/85 hover:bg-white/5">{isFullscreen ? <FiMinimize2 className="h-4 w-4 shrink-0"/> : <FiMaximize2 className="h-4 w-4 shrink-0"/>}<span className="flex-1">{isFullscreen ? 'Exit full screen' : 'Full screen'}</span></Button><Button variant="custom" size="none" type="button" onClick={() => { navigator.clipboard?.writeText(`${window.location.origin}/meetings?room=${encodeURIComponent(roomName)}`); setShowMore(false); }} className="flex w-full items-center gap-3 border-t border-white/5 px-4 py-3 text-left text-sm text-white/85 hover:bg-white/5"><FiCopy className="h-4 w-4 shrink-0"/><span className="flex-1">Copy meeting link</span></Button></Card></>}</div><Button variant="custom" size="none" onClick={leaveVoluntarily} aria-label="Leave meeting" title="Leave meeting" className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-red-500 text-white shadow-lg shadow-red-950/30 hover:bg-red-400"><FiPhoneOff/></Button></div></div>
...
function ParticipantTile({ participant, local, speaking, pinned, onTogglePin, big }) {
  const videoRef = useRef(null); const audioRef = useRef(null); const [, rerender] = useState(0); const [avatarFailed, setAvatarFailed] = useState(false);
  useEffect(() => { const sync = () => rerender(v => v + 1); participant.on?.(RoomEvent.TrackPublished, sync).on?.(RoomEvent.TrackUnpublished, sync).on?.(RoomEvent.TrackSubscribed, sync).on?.(RoomEvent.TrackUnsubscribed, sync).on?.(RoomEvent.TrackMuted, sync).on?.(RoomEvent.TrackUnmuted, sync).on?.(ParticipantEvent.ConnectionQualityChanged, sync); return () => { participant.off?.(RoomEvent.TrackPublished, sync).off?.(RoomEvent.TrackUnpublished, sync).off?.(RoomEvent.TrackSubscribed, sync).off?.(RoomEvent.TrackUnsubscribed, sync).off?.(RoomEvent.TrackMuted, sync).off?.(RoomEvent.TrackUnmuted, sync).off?.(ParticipantEvent.ConnectionQualityChanged, sync); }; }, [participant]);
  useEffect(() => { const video = participant.getTrackPublication?.(Track.Source.Camera)?.track; const screen = participant.getTrackPublication?.(Track.Source.ScreenShare)?.track; const track = screen || video; if (track && videoRef.current) track.attach(videoRef.current); const audioTrack = participant.getTrackPublication?.(Track.Source.Microphone)?.track; if (audioTrack && audioRef.current) audioTrack.attach(audioRef.current); return () => { try { track?.detach(videoRef.current); } catch {} try { audioTrack?.detach(audioRef.current); } catch {} }; });
  const display = participant.name || participant.identity || 'Participant'; const hasScreenShare = Boolean(participant.getTrackPublication?.(Track.Source.ScreenShare)?.track); const hasVideo = hasScreenShare || Boolean(participant.getTrackPublication?.(Track.Source.Camera)?.track); const micOn = Boolean(participant.isMicrophoneEnabled); const avatarPalette = ['#8A2BE2', '#EC2FA8', '#1a73e8', '#F2A31C', '#34D399', '#E85D75']; const avatarColor = avatarPalette[String(display).split('').reduce((sum, ch) => sum + ch.charCodeAt(0), 0) % avatarPalette.length]; let avatarUrl = null; try { avatarUrl = JSON.parse(participant.metadata || '{}')?.avatarUrl || null; } catch { /* malformed metadata — fall back to initial */ }
...
function ReactionsBar({ liveRoom, localParticipant, participants }) {
  ...
  useEffect(() => { if (!liveRoom) return undefined; const handler = (payload, participant, _kind, topic) => { try { const data = JSON.parse(decoder.current.decode(payload)); if (topic === 'reactions') popBubble(data.emoji, data.name); else if (topic === 'raise-hand') setRaisedHands((prev) => { const next = { ...prev }; const id = participant?.identity || data.identity; if (data.raised) next[id] = data.name; else delete next[id]; return next; }); } catch { /* ignore malformed payload */ } }; liveRoom.on(RoomEvent.DataReceived, handler); return () => liveRoom.off(RoomEvent.DataReceived, handler); }, [liveRoom, popBubble]);
  ...
  const react = (emoji) => { const name = localParticipant?.name || 'BLW Member'; popBubble(emoji, name); localParticipant?.publishData(new TextEncoder().encode(JSON.stringify({ emoji, name })), { reliable: true, topic: 'reactions' }); setShowPicker(false); };
  const toggleHand = () => { const next = !handRaised; setHandRaised(next); const name = localParticipant?.name || 'BLW Member'; localParticipant?.publishData(new TextEncoder().encode(JSON.stringify({ raised: next, name, identity: localParticipant?.identity })), { reliable: true, topic: 'raise-hand' }); };
  const raisedList = Object.values(raisedHands);
  return <><div className="pointer-events-none fixed inset-x-0 bottom-24 z-20 h-44 overflow-hidden">{bubbles.map((b) => <div key={b.id} className="reaction-bubble absolute bottom-0 text-2xl drop-shadow-lg" style={{ left: `${b.left}%` }}><span>{b.emoji}</span><span className="ml-1 rounded-full bg-[#11101d]/75 px-2 py-0.5 align-middle text-[10px] text-white/80 backdrop-blur">{b.name}</span></div>)}</div>{raisedList.length > 0 && <div className="fixed bottom-24 left-1/2 z-40 -translate-x-1/2 rounded-full border border-pink-400/25 bg-[#1a1926]/90 px-4 py-1.5 text-xs text-pink-200 shadow-lg backdrop-blur">✋ {raisedList.join(', ')} raised {raisedList.length === 1 ? 'a hand' : 'hands'}</div>}{showPicker && <><Button variant="custom" size="none" type="button" aria-label="Close reaction picker" onClick={() => setShowPicker(false)} className="fixed inset-0 z-40 cursor-default !bg-transparent !border-0 !p-0"/><Card variant="custom" className="fixed bottom-[5.8rem] left-1/2 z-50 -translate-x-1/2 rounded-full border border-purple-400/20 bg-[#11101d]/98 p-1.5 shadow-2xl shadow-purple-900/20 backdrop-blur-xl"><div className="flex items-center gap-1">{REACTION_EMOJIS.map((emoji) => <Button variant="custom" size="none" key={emoji} type="button" onClick={() => react(emoji)} aria-label={`React with ${emoji}`} className="grid h-11 w-11 place-items-center rounded-full bg-white/[0.06] px-0 py-0 text-xl shadow-inner ring-1 ring-white/5 transition hover:scale-105 hover:bg-purple-500/20">{emoji}</Button>)}</div></Card></>}<div className="mt-2 flex justify-center">{handRaised && <span className="sr-only">Your hand is raised</span>}</div><style>{`@keyframes reaction-float { 0% { transform: translateY(0) scale(.92); opacity: 0; } 12% { opacity: 1; } 100% { transform: translateY(-180px) scale(1.06); opacity: 0; } }.reaction-bubble { animation: reaction-float 2.5s cubic-bezier(.2,.75,.25,1) forwards; }`}</style></>;
}
...
function HostPanel({ roomName, open, onClose, participants, locked, setLocked, waitingRoomEnabled, setWaitingRoomEnabled, recording, onRecordingChange, onEnded }) {
  const [busy, setBusy] = useState(''); const [notice, setNotice] = useState(''); const [log, setLog] = useState([]); const [recordings, setRecordings] = useState([]); const closeButtonRef = useRef(null);
  useEffect(() => { if (!open) return; closeButtonRef.current?.focus(); (async () => { try { const res = await apiFetch(`/api/video/rooms/${encodeURIComponent(roomName)}/status`); const body = await res.json().catch(() => ({})); if (res.ok) setLog(Array.isArray(body.log) ? body.log : []); } catch {} try { const res = await apiFetch(`/api/video/rooms/${encodeURIComponent(roomName)}/recordings`); const body = await res.json().catch(() => ({})); if (res.ok) setRecordings(Array.isArray(body.recordings) ? body.recordings : []); } catch {} })(); }, [open, roomName]);
  if (!open) return null;
  const waitingParticipants = participants.filter(isPendingParticipant); const activeParticipants = participants.filter((p) => !isPendingParticipant(p));
  const call = async (path, options) => { const response = await apiFetch(path, { method: 'POST', ...options }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body?.error || 'That action failed.'); return body; };
