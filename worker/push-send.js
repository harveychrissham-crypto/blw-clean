...
async function verifyAdmin(request, env) {
  ...
  try {
    ...
    try {
      ...
      if (result.rows[0]?.is_admin !== true) return { ok: false, status: 403, error: 'Administrator authorization is required.' };
    } finally {
...
async function getFirebaseAccessToken(env) {
  ...
  if (!response.ok || !body.access_token) throw new Error(body.error_description || body.error || 'Unable to obtain Firebase access token.');
  return body.access_token;
}

async function sendToken({ token, title, body, accessToken, projectId }) {
  const response = await fetch(`https://fcm.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/messages:send`, {
    ...
    body: JSON.stringify({
      message: {
        ...
        android: { priority: 'HIGH', notification: { channel_id: 'blw_default', sound: 'default' } },
      },
    }),
  });
...
export async function sendPushNotification(request, env) {
  ...
  try {
    ...
    try {
      ...
      const result = broadcast ? await client.query(sql) : await client.query(sql, [userEmails]);
