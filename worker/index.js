...
const sanitizeString = (value) => {
  ...
  return value.trim().replace(/<[^>]*>/g, '').replace(/[<>\"']/g, (c) => ({
    '<': '&lt;',
    '>': '&gt;',
    '"': '&quot;',
    "'": '&#39;',
...
async function handleAuthApi(request, workerEnv, url) {
  ...
  if (url.pathname === '/api/auth/register') {
    ...
    if (!fullName || !email || !password || !phone || !campusZone || !chapter || !country || !residence || !invitedBy || !gender) {
      return json({ error: 'Missing required registration fields.' }, 400, corsHeaders);
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    try {
      const result = await withDb(workerEnv, async (client) => {
        ...
        const inserted = await client.query(
          `INSERT INTO users (
             full_name, email, password_hash, phone, campus_zone, chapter,
             country, residence, birthday, invited_by, gender, membership_id, badge, status
