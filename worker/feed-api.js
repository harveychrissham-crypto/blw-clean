...
async function getDb(env) { const connectionString = env.HYPERDRIVE?.connectionString || env.DATABASE_URL || ''; if (!connectionString) throw new Error('Database connection is not configured.'); const { Client } = await import('pg'); const client = new Client({ connectionString, connectionTimeoutMillis: 8000, query_timeout: 10000 }); await client.connect(); return client; }
...
async function notifyMentions(client, actorEmail, text, postId) {
  const matches = typeof text === 'string' ? text.match(/@[A-Za-z0-9_]{2,40}/g) || [] : [];
  const handles = [...new Set(matches.map((item) => item.slice(1).toLowerCase()))];
  if (!handles.length) return;
  for (const handle of handles) {
    const recipient = await client.query(
      "SELECT email FROM public.users WHERE LOWER(regexp_replace(trim(full_name), '[^A-Za-z0-9]+', '_', 'g')) = $1 LIMIT 1",
      [handle],
    );
    const recipientEmail = String(recipient.rows[0]?.email || '').trim().toLowerCase();
    if (!recipientEmail || recipientEmail === actorEmail) continue;
    await client.query(
      "INSERT INTO public.notifications(recipient_email,actor_email,type,post_id,body) VALUES($1,$2,'mention',$3,$4)",
      [recipientEmail, actorEmail, postId, text.slice(0, 500)],
    );
  }
}
...
async function recentLikerNames(client, table, column, id) { const result = await client.query(`SELECT u.full_name AS name, u.avatar_url AS avatar_url FROM public.${table} l JOIN public.users u ON LOWER(u.email) = LOWER(l.user_email) WHERE l.${column} = $1 ORDER BY l.created_at DESC LIMIT 2`, [id]); return result.rows.map((row) => ({ name: String(row.name || '').trim(), avatarUrl: String(row.avatar_url || '').trim() })).filter((row) => row.name); }
  ...
  try {
    ...
    if (url.pathname === '/api/feed' && request.method === 'GET') {
      ...
      try {
        ...
        const result = await client.query(`SELECT * FROM (
          ...
          qp.author_name AS quoted_author,
          qp.body AS quoted_body,
          qp.media_url AS quoted_media_url,
          qp.media_type AS quoted_media_type,
          qp.youtube_url AS quoted_youtube_url,
          ...
          (SELECT COUNT(*)::int FROM public.feed_post_reposts x WHERE x.post_id=p.id) AS repost_count,
          ...
          CASE WHEN $1 <> '' AND EXISTS(SELECT 1 FROM public.feed_post_reposts x WHERE x.post_id=p.id AND LOWER(x.user_email)=$1) THEN true ELSE false END AS reposted,
    ...
    if (url.pathname === '/api/feed/posts' && request.method === 'POST') {
      ...
      try {
        ...
        await notifyMentions(client, email, `${title} ${text}`, `p:${post.id}`);
    ...
    const match = url.pathname.match(/^\/api\/feed\/posts\/([^/]+)(?:\/(like|save|repost|quote|comments|report))?$/);
    ...
    try {
      if (feedId.kind === 'post') {
        ...
        if(!action&&request.method==='DELETE'){if(!email)return json({error:'Sign in required.'},401,headers);const deleted=await client.query('DELETE FROM public.feed_posts WHERE id=$1 AND LOWER(user_email)=$2 RETURNING id',[feedId.id,email]);if(!deleted.rows.length)return json({error:'Post not found, or it is not yours to delete.'},404,headers);return json({ok:true},200,headers);}
        ...
        if(action==='repost'&&request.method==='POST'){if(!email)return json({error:'Sign in required to repost.'},401,headers);if(feedId.kind!=='post')return json({error:'This content cannot be reposted yet.'},400,headers);const current=await client.query('SELECT 1 FROM public.feed_post_reposts WHERE post_id=$1 AND LOWER(user_email)=$2 LIMIT 1',[feedId.id,email]);if(current.rows.length)await client.query('DELETE FROM public.feed_post_reposts WHERE post_id=$1 AND LOWER(user_email)=$2',[feedId.id,email]);else await client.query('INSERT INTO public.feed_post_reposts(post_id,user_email) VALUES($1,$2) ON CONFLICT(post_id,user_email) DO NOTHING',[feedId.id,email]);return json(await postSocialCounts(client,feedId.id,email),200,headers);}if(action==='quote'&&request.method==='POST'){if(!email)return json({error:'Sign in required to quote posts.'},401,headers);if(feedId.kind!=='post')return json({error:'Only Emet posts can be quoted right now.'},400,headers);const body=await request.json().catch(()=>({}));const text=typeof body?.body==='string'?body.body.trim().slice(0,5000):'';if(!text)return json({error:'Add a comment to your quote.'},400,headers);const source=await client.query('SELECT id,title,body,author_name,user_email,media_url,media_type,youtube_url FROM public.feed_posts WHERE id=$1 LIMIT 1',[feedId.id]);if(!source.rows.length)return json({error:'Post not found.'},404,headers);const name=await authorName(client,email);const inserted=await client.query('INSERT INTO public.feed_posts (user_email,author_name,type,title,body,quoted_post_id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id,user_email,author_name,type,title,body,quoted_post_id,created_at',[email,name,'post',text.slice(0,160),text,feedId.id]);const post=inserted.rows[0];await notifyMentions(client,email,text,`p:${post.id}`);return json({post:{...post,id:`p:${post.id}`,source_type:'user',is_user_post:true,is_owner:true,quoted_post_id:feedId.id,quoted_title:source.rows[0].title,quoted_author:source.rows[0].author_name,quoted_body:source.rows[0].body,quoted_media_url:source.rows[0].media_url,quoted_media_type:source.rows[0].media_type,quoted_youtube_url:source.rows[0].youtube_url,youtube_id:youtubeId(post.youtube_url),like_count:0,save_count:0,comment_count:0,save_count:0,repost_count:0,view_count:0,liked:false,saved:false,reposted:false}},201,headers);}
