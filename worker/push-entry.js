...
import { handleCommunities } from './community-api.js';

function normalizeResponse(request, env, response) {
  const headers = new Headers(response.headers);
  headers.delete('access-control-allow-origin'); headers.delete('access-control-allow-credentials'); headers.delete('access-control-allow-methods'); headers.delete('access-control-allow-headers'); headers.delete('vary');
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/api/communities')) { const response = await handleCommunities(request, env, url); if (response) return normalizeResponse(request, env, response); }
