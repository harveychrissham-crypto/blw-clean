  ...
  try {
    const result = await db(env, async (client) => {
      ...
      if (url.pathname === '/api/live' && request.method === 'PUT') {
        ...
        const r = await client.query(`UPDATE live_stream SET title=$1,youtube_url=$2,google_meet_url=$3,daily_room_url=$4,is_live=$5,updated_at=NOW() WHERE id=1 RETURNING *`, [body.title || '', youtubeUrl, googleMeetUrl, dailyRoomUrl, !!body.isLive]);
        return { status: 200, body: { live: liveDto(r.rows[0]) } };
      }
      if (url.pathname === '/api/live/viewers' && request.method === 'GET') {
        ...
        return { status: 200, body: { viewers: r.rows.map(v => ({ id:v.id,name:v.name,invitedBy:v.invited_by || '',createdAt:v.created_at,lastSeenAt:v.last_seen_at,visitCount:v.visit_count || 1,watchSeconds:v.watch_seconds || 0 })) } };
      }
      if (url.pathname === '/api/live/viewers' && request.method === 'POST') {
        ...
        const r = clientId
          ? await client.query(`INSERT INTO live_viewers (name,invited_by,client_id,last_seen_at,visit_count) VALUES ($1,$2,$3,NOW(),1) ON CONFLICT (client_id) DO UPDATE SET name=EXCLUDED.name,invited_by=CASE WHEN EXCLUDED.invited_by<>'' THEN EXCLUDED.invited_by ELSE live_viewers.invited_by END,last_seen_at=NOW(),visit_count=live_viewers.visit_count+1 RETURNING *`, [name, invitedBy, clientId])
          : await client.query('INSERT INTO live_viewers (name,invited_by) VALUES ($1,$2) RETURNING *', [name, invitedBy]);
        const v = r.rows[0];
        return { status: 201, body: { viewer: { id:v.id,name:v.name,invitedBy:v.invited_by || '',createdAt:v.created_at,lastSeenAt:v.last_seen_at,visitCount:v.visit_count || 1,watchSeconds:v.watch_seconds || 0 } } };
      }
      if (url.pathname === '/api/live/viewers/heartbeat' && request.method === 'PATCH') {
        ...
        const r = await client.query('UPDATE live_viewers SET watch_seconds=watch_seconds+$1,last_seen_at=NOW() WHERE client_id=$2 RETURNING *', [Math.min(Math.round(seconds),300), clientId]);
        if (!r.rows.length) return { status: 204, body: null };
        const v = r.rows[0];
        return { status: 200, body: { viewer: { id:v.id,name:v.name,invitedBy:v.invited_by || '',createdAt:v.created_at,lastSeenAt:v.last_seen_at,visitCount:v.visit_count || 1,watchSeconds:v.watch_seconds || 0 } } };
      }
      return { status: 405, body: { error: 'Method not allowed.' } };
    });
    return result.status === 204 ? new Response(null, { status: 204, headers }) : json(result.body, result.status, headers);
  } catch (error) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    const live = await handleLive(request, env, url);
    if (live) return live;
    if (url.pathname === '/api/health' && request.method === 'GET') {
      return json({ status: 'ok', message: 'BLW Campus Ministry API is running' }, 200, cors(url.origin));
    }
