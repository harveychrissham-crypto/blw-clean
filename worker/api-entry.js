...
const authEmail = async (request, env) => {
  const token = bearerToken(request);
  if (!token) return '';
  const jwtSecret = env.JWT_SECRET || env.DATABASE_URL || '';
  if (!jwtSecret) return '';
  try {
...
const memberDto = (m) => ({
  name: m.full_name, email: m.email, phone: m.phone, campusZone: m.campus_zone, chapter: m.chapter,
  status: m.status, badge: m.badge, membershipId: m.membership_id,
  joinDate: m.joined_date ? new Date(m.joined_date).toISOString().slice(0, 10) : '',
  country: m.country || '', residence: m.residence || '',
  birthday: m.birthday instanceof Date ? m.birthday.toISOString().slice(0, 10) : (m.birthday || ''),
  gender: m.gender || '', invitedBy: m.invited_by || '', checkedIn: Boolean(m.checkin_id),
...
const selectMembers = `SELECT ${memberFields}, c.id AS checkin_id, c.checked_in_at AS checkin_at, c.checked_in_by AS checkin_by
  FROM users u LEFT JOIN checkins c
  ...
  try {
    const result = await db(env, async (client) => {
      ...
      if (url.pathname === '/api/members/search' && request.method === 'GET') {
        ...
        const normalized = raw.toLowerCase(); const digits = raw.replace(/[\s\-()]/g, '');
        ...
        return { status: 200, body: { members: r.rows.map(memberDto), checkedIn: false } };
      }
      if (url.pathname === '/api/members' && request.method === 'GET') {
        const r = await client.query(`${selectMembers} ORDER BY u.created_at DESC`);
      ...
      if (match && request.method === 'POST') {
        ...
        const email = await authEmail(request, env);
        const eventBody = await request.json().catch(() => ({}));
        const eventId = eventBody?.eventId == null || eventBody.eventId === '' ? null : Number(eventBody.eventId);
        ...
        try {
          const inserted = await client.query(`INSERT INTO checkins (membership_id, attendance_date, event_id, checked_in_at, checked_in_by) VALUES ($1, ((NOW() AT TIME ZONE 'Africa/Nairobi')::date), $2, NOW(), $3) RETURNING id, checked_in_at, checked_in_by`, [membershipId, eventId, email || null]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    const memberResponse = await handleMembers(request, env, url);
