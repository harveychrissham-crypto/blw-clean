...
const parseLocation = (body) => {
  ...
  const country = clean(body?.country, 80) || 'Kenya';
  const city = clean(body?.city, 100), town = clean(body?.town, 100), area = clean(body?.area, 120);
  const university = clean(body?.university, 180), address = clean(body?.address, 240);
  const description = clean(body?.description, 500), serviceTime = clean(body?.serviceTime, 100);
...
async function handle(request, env, url) {
  ...
  if (url.pathname === '/api/geocode' && request.method === 'GET') {
    ...
    try {
      ...
      nominatim.searchParams.set('q', query); nominatim.searchParams.set('format', 'jsonv2');
      nominatim.searchParams.set('addressdetails', '1'); nominatim.searchParams.set('limit', '6');
  ...
  try {
    if (url.pathname.startsWith('/api/fellowships/admin') && !(await validAdmin(request, env))) {
      return json({ error: 'Administrator authorization is required.' }, 403, headers);
    }
    ...
    const result = await db(env, async (client) => {
      ...
      if (request.method === 'GET' && !adminRoute) {
        ...
        if (nearby) {
          const cf = request.cf || {}, latitude = Number(cf.latitude), longitude = Number(cf.longitude);
      ...
      if (request.method === 'POST' && url.pathname === '/api/fellowships/admin') {
        const parsed = parseLocation(await request.json().catch(() => null));
      ...
      if (match) {
        ...
        if (request.method === 'DELETE') {
          ...
          return rows.rows.length ? { status: 200, body: { deleted: true, id } } : { status: 404, body: { error: 'Fellowship location not found.' } };
        }
        if (request.method === 'PUT' || request.method === 'PATCH') {
          const parsed = parseLocation(await request.json().catch(() => null));
          if (parsed.error) return { status: 400, body: { error: parsed.error } };
          const v = parsed.value;
          const rows = await client.query(`UPDATE chapter_venues SET chapter=$1,venue=$2,service_time=$3,fellowship_name=$1,country=$4,city=$5,town=$6,area=$7,university=$8,address=$9,description=$10,latitude=$11,longitude=$12,is_active=$13,updated_at=NOW() WHERE id=$14 RETURNING *`, [v.fellowshipName, v.address || v.fellowshipName, v.serviceTime, v.country, v.city, v.town, v.area, v.university, v.address, v.description, v.latitude, v.longitude, v.isActive, id]);
          return rows.rows.length ? { status: 200, body: { fellowship: locationDto(rows.rows[0]) } } : { status: 404, body: { error: 'Fellowship location not found.' } };
        }
      }
    ...
    console.error('[worker] fellowships API failed', { message: error?.message, code: error?.code, path: url.pathname, method: request.method });
    return json({ error: error?.message || 'Unable to access fellowship locations right now.' }, 503, headers);
  }
}
