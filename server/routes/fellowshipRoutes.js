...
const DEFAULT_CODE = '1120363';
const clean = (v, max = 240) => typeof v === 'string' ? v.trim().replace(/[<>]/g, '').slice(0, max) : '';
const secret = () => process.env.JWT_SECRET || `blw-leader-auth:${process.env.FELLOWSHIP_ADMIN_ACCESS_CODE || DEFAULT_CODE}`;
...
router.post('/admin/auth', (req,res) => {
  const supplied=clean(req.body?.accessCode,80), expected=process.env.FELLOWSHIP_ADMIN_ACCESS_CODE || DEFAULT_CODE;
  if(!supplied || supplied!==expected) return res.status(401).json({error:'Invalid leadership access code.'});
  res.json({token:jwt.sign({leaderAdmin:true},secret(),{expiresIn:'8h'})});
});
router.use('/admin', (req,res,next) => { if(!isLeader(req)) return res.status(403).json({error:'Valid leadership is required.'}); corsHeaders(res); next(); });
