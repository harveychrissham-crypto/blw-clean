...
// Route modules are intentionally imported normally so the same application
// remains usable locally on Node.js and in Cloudflare Workers.
...
export function createApp({ serveStatic = false } = {}) {
  ...
  // For the Cloudflare deployment the browser normally uses same-origin /api
  // requests. CORS is still kept for the Capacitor mobile app and local dev.
...
// Keep the existing Node/Render workflow working when this file is run directly.
